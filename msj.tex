...
To describe the mapping, consider an arbitrary $(k-1)$-tuple $\bm{t} = (r_1,...,$ $r_{k-3},$ $h', v) \in \J(R_1,...,$ $R_{k-3},H',V)$. Let $h$ be the full segment of $h'$. By the definition of trimmed segments (see Section~\ref{sec:hv:type2}), at least one rectangle $r_{k-2} \in R_{k-2}$ covers both the left endpoint of $h$ and the right endpoint of $h'$. We map $\bm{t}$ to the $k$-tuple $(r_1,...,r_{k-3}, r_{k-2},h,v)$. It is worth noting that $r_{k-2}$ definitely contains the entire $h'$ (which has the same left point as $h$, implying that $r_{k-2}$ covers both endpoints of $h'$).

\vgap

We argue that $(r_1,...,r_{k-3}, r_{k-2},h,v)$ belongs to $\J_2$. Let $p$ be the intersection point of $h'$ and $v$. As $h'$ is contained in $h$, the intersection point between $h$ and $v$ is also $p$. Since $(r_1,...,r_{k-3}, h', v) \in \J(R_1,...,R_{k-3},H',V)$, it must hold that $p$ falls in $r_i$ for $i \in [k-3]$. From our earlier discussion, we already know that $r_{k-2}$ contains $h'$; hence, $p$ falls in $r_{k-2}$ as well. Thus, $h \cap v \cap \bigcap_{i = 1}^{k-2}r_i$ is non-empty, indicating that $(r_1,...,r_{k-3}, r_{k-2},h,v) \in \J(R_1,...,R_{k-2},H,V)$. This, together with the fact that $r_{k-2}$ covers the left endpoint of $h$, tells us $(r_1,...,r_{k-3}, r_{k-2},h,v) \in \J_2$.
...
\extraspacing \underline{\em Proof of Statement (1).} Consider any tuple $\bm{t} = (r_1,...,r_{k-3}, h', v) \in \J(R_1,...,R_{k-3},H',V)$. Let $h$ be the full segment of $h'$. The rectangle $B_\bm{t}$ defined in \eqref{eqn:B_t} is the intersection point of $h'$ and $v$ (this point is covered by all of $r_1,...,r_{k-3}$). As $h$ is the full segment of $h'$, the point $B_\bm{t}$ must also be the intersection point of $h$ and $v$.

\vgap

Let $r_{k-2}$ be an arbitrary rectangle in the set $\contained_{R_{k-2}}(\bm{t})$. The definition of $\contained_{R_{k-2}}(\bm{t})$ tells us that $r_{k-2}$ covers the effective horizontal segment of $h'$ (see Section~\ref{sec:hv:type2} for the definition of ``effective horizontal segment''), whose right endpoint is $B_{\bm{t}}$. This means that $B_\bm{t}$ falls in $r_{k-2}$ as well.

\vgap

Now we know that $h \cap v \cap \bigcap_{i = 1}^{k-2} r_i$ contains $B_\bm{t}$ and is thus non-empty. Hence,
$(r_1,...,r_{k-3}, r_{k-2}, h, v) \in \J(R_1,...,R_{k-2},H,V)$. To show $(r_1,...,r_{k-3}, r_{k-2}, h, v) \in \J_2$, we still need to prove that $r_{k-2}$ covers the left endpoint of $h$. This follows immediately from the fact that $r_{k-2}$ covers the effective horizontal segment of $h'$ (which shares the same left endpoint as $h$).
