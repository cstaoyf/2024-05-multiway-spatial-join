...
\acmJournal{PACMMOD}
\acmYear{2024} \acmVolume{2} \acmNumber{5 (PODS)} \acmArticle{210} \acmMonth{11}\acmDOI{10.1145/3695828}

\received{May 2024}
\received[accepted]{August 2024}
...
\titlenote{This work was supported in part by GRF projects 14207820, 14203421, and 14222822 from HKRGC.}

\author{Ru Wang}
\email{rwang21@cse.cuhk.edu.hk}
\author{Yufei Tao}
...
\keywords{Multiway Spatial Joins; Computational Geometry; Theory}
...
In \cite{kcko22}, Khamis et al.\ investigated a type of joins that extends the conventional equi-join in two ways. First, each attribute value in a relation is an interval (rather than a real value); second, each equality predicate in an equi-join is replaced with a ``non-empty intersection'' predicate on the attributes involved. The $k$-SJ problem can be converted to a join under the framework of \cite{kcko22} as defined next. For each $i \in [k]$, define $R_i$ as a relation over two attributes $X$ and $Y$. For each tuple $\bm{t} \in R_i$, its values $\bm{t}(X)$ and $\bm{t}(Y)$ on the two attributes are both intervals (effectively defining a rectangle). The objective is to output all $k$-tuples $(\bm{t}_1, \bm{t}_2, ..., \bm{t}_k) \in R_1 \times R_2 \times ... \times R_k$ satisfying $\bigcap_{i=1}^k \bm{t}_i(X) \ne \emptyset$ and $\bigcap_{i=1}^k \bm{t}_i(Y) \ne \emptyset$. It is clear that there is one-one correspondence between the result of this join and that of k-SJ. Khamis et al.\ \cite{kcko22} developed an algorithm that can process the join  in $O(n \log^{2k-1} n + \out)$ time.

\vgap 

$\Omega(n \log n)$ is a lower bound on the runtime of any comparison-based $k$-SJ algorithms
%(see Appendix~\ref{app:comp-class} for a formal definition of such algorithms),
even for $k = 2$. This can be established via a reduction from the {\em element distinctness} problem; see \cite{dl79}.
...
The theorem implies a recursive nature of $k$-SJ. Indeed, we will see that an $k$-SJ instance with input size $n$ and output size $\out$ can be converted to $O(k^3)$ instances of the $(k-1)$-SJ problem --- all having input size at most $n$ and output size at most $\out$ --- plus an additional cost of $O(k^3) \cdot (n \log n + k \cdot \out)$. For 2-SJ, we can set $\A$  to the ``folklore algorithm'' mentioned in Section~\ref{sec:intro:prev}, which ensures $F_2(n, \out) = O(n \log n + \out)$. Combining this with \eqref{eqn:main:reccurrence} gives a recurrence that relates the time complexity of $k$-SJ to that of $(k-1)$-SJ. Solving the recurrence yields:
...
\begin{table}
    \begin{tabular}{c|c|c|c}
        ...
        $\ge 3$ & \cite{kcko22} & $O(n \log^{2k-1} n + \out)$ & \\
        \hline
...
Given a horizontal segment $h = [x_1, x_2] \times y$, we say that a rectangle $r$ is a {\em left-end covering rectangle} of $h$ if $r$ contains the left endpoint of $h$ (i.e., $(x_1, y) \in r$). A horizontal/vertical segment $s$ {\em crosses} a rectangle $r$ if $s \cap r \ne \emptyset$ but $r$ covers neither of the two endpoints of $s$. A rectangle $r$ {\em contains} a horizontal/vertical segment $s$ if $r$ covers both endpoints of $s$.
...
namely, $\cross_S(r)$ is the set of segments in $S$ crossing $r$.
Let $R$ be a set of rectangles. Given a horizontal segment $h$, we define 
\myeqn{
    \contain_R(h) = \{r \in R \mid \text{$r$ contains $h$} \};
    \label{eqn:contained}
}
namely, $\contain_R(h)$ is the set of rectangles in $R$ containing $h$.


\begin{figure}
    \includegraphics[height=25mm]{./artwork/guard}

    \figcapup
    \caption{For 4-tuple $\bm{t} = \set{r_1, r_2, r_3, r_4}$, $B_\bm{t}$ is the rectangle in gray, $\gleft(\bm{t}) = r_3$ and $\gbot(\bm{t}) = r_2$.}
    \label{fig:guard}
    \figcapdown
\end{figure}
...
\myeqn{
    B_\bm{t} &=& \bigcap_{i=1}^{k} r_i; \label{eqn:B_t}
}
namely, $B_\bm{t}$ is the intersection of the rectangles in $\bm{t}$ (note: $B_\bm{t}$ is a rectangle itself). Also, if $B_\bm{t}$ is not empty, define:
\myitems{
...
\begin{figure*}
    \begin{tabular}{ccccc}
        \includegraphics[height=23mm]{./artwork/prob-a} &
        \includegraphics[height=23mm]{./artwork/prob-b} &
        \includegraphics[height=23mm]{./artwork/prob-c} &
        \includegraphics[height=23mm]{./artwork/prob-d} &
        \includegraphics[height=25mm]{./artwork/prob-e} \\[2mm]
        (a) Problem $\mathscr{A}$ &
        (b) Problem $\mathscr{B}$ &
        (c) Problem $\mathscr{C}$ &
        (d) Problem $\mathscr{D}$ &
    ...
    \end{tabular}
    \figcapup
    \caption{Five geometric building brick problems}
    \label{fig:probs}
    \figcapdown
\end{figure*}
...
\extraspacing {\bf Problem $\bm{\mathscr{E}}$.} The input involves a set $H$ of horizontal segments and a set $R$ of rectangles. The goal is to report, for each segment $h \in H$, the set $\contain_R(h)$ --- defined in \eqref{eqn:contained} --- where the rectangles are sorted by their right boundaries; if $\contain_R(h)$ is empty, output nothing for $h$. Formally, if $r_1, r_2, ..., r_z$ for some $z \ge 1$ are all the rectangles in $\contain_R(h)$, we output $(h: r_1, r_2, ..., r_z)$, provided that $\xright(r_i) \ge \xright(r_{i-1})$ for each $i \in [2, z]$. Figure~\ref{fig:probs}e gives an example where $H = \set{h_1, h_2, h_3}$ and $R = \set{r_1, r_2, r_3}$; the output is $\set{(h_1: r_2, r_1), (h_2: r_3, r_2)}$. In Appendix~\ref{app:bricks}, we explain how to solve the problem in $O(n \log n + \out)$ time where $n = |H| + |R|$ and $\out$ is the number of pairs $(h, r) \in H \times R$ such that $r$ contains $h$.
...
\myitems{
    ...
    at least a rectangle $r_i$ (for some $i \in [k-2]$) covers an endpoint of either $h$ or $v$ or both.
}
...
\extraspacing {\underline{\em Example 4.1.}} To illustrate our algorithm, we will utilize the running example in Figure~\ref{fig:hv:type1:ex}a, where $k = 4$, and $R_1 = \set{\alpha}$ (the solid rectangle), $R_2 = \set{\beta_1, \beta_2}$ (the dashed rectangles), $H$ $=$ $\set{h_1,$ $h_2,$ $..., h_6}$, and $V = \set{v_1, v_2, ..., v_5}$. The set $\J_1$ contains the following tuples: $(\alpha, \beta_2, h_2, v_3)$, $(\alpha, \beta_2, h_2, v_5)$, $(\alpha, \beta_2, h_5, v_3)$, and $(\alpha,$ $\beta_2, h_5, v_5)$. \done
...
Construct
\myeqn{
    R_i' &=& \set{r' \mid r \in R_i \text{ and its trimmed rectangle $r'$ exists}}. \label{eqn:hv:type1:R_i'}
}
Computing the ``segment $h_\bot$'' for each $r \in R_i$ is an instance of Problem $\mathscr{D}$ (the find-lowest version, with $H$ and $R_i$ as the input). By symmetry, so is computing the $h_\top, v_\vdash$, and $v_\dashv$ segments for each $r \in R_i$. It thus follows from Section~\ref{sec:bricks} that $R_1', R_2', ..., R_{k-2}'$ can be produced in $O(kn \log n)$ total time.
...
\extraspacing {\underline{\em Example 4.2.}} Figure~\ref{fig:hv:type1:ex}b shows the rectangles in $R'_1 = \set{\alpha'}$ and $R'_2$ $=$ $\{\beta'_1,$ $\beta'_2\}$. For instance, $\alpha'$, which is trimmed from rectangle $\alpha$, is decided by $h_\bot$ $=$ $h_2$, $h_\top$ $=$ $h_6$, $v_\vdash = v_3$, and $v_\dashv = v_5$. The $(k-2)$-SJ on $R'_1$ and $R'_2$ returns $\J(R_1', R'_2) = \set{(\alpha', \beta_1'), (\alpha', \beta_2')}$.
\done
...
\begin{lemma} \label{lmm:hv:type1:properties}
    ...
    \myenums{
        ...
        \item Consider any $k$-tuple $(r_1, r_2, ..., r_{k-2}, h, v) \in \J_1$. Let $r'_i$ ($i \in [k-2])$ be the trimmed rectangle of $r_i$, and set $\bm{t} = (r'_1, r'_2, ...,$ $r'_{k-2})$. Then, we must have
        \myitems{
...
By statements (1) and (2) of Lemma~\ref{lmm:hv:type1:properties}, the set $\J^*$ thus computed indeed satisfies \eqref{eqn:hv:type1:J*}. Furthermore, if we are given $\dcross_H(\bm{t})$ and $\dcross_V(\bm{t})$ for each $\bm{t}$, the above algorithm runs in $O(1 + k \cdot |\J(R'_1, ..., R'_{k-2})| + k \cdot |\J^*|) = O(1 + k \cdot \out)$ time, where the derivation used \eqref{eqn:hv:type1:J*} and Lemma~\ref{lmm:hv:type1:recur-output}.

\vgap 

The rest of the section will focus on how to prepare the sets $\dcross_H(\bm{t})$ of all $\bm{t} \in \J(R_1', ..., R_{k-2}')$ in $O(k n \log n + k \cdot \out)$ time. An analogous method can be used to compute the sets $\dcross_V(\bm{t})$ of all $\bm{t}$ within the same time complexity.
...
In Appendix~\ref{app:hv:type1}, we show how to compute $R^*_1, ..., R^*_{k-2}$ altogether in $O(n + k \cdot \out)$ total time.

\vgap 

Our interest lies specifically in the sets $\cross_H(r^*)$ of the rectangles $r^*$ in $R^*_i$, where $\cross_H(r^*)$ --- defined in \eqref{eqn:cross} --- is the set of segments in $H$ crossing $r^*$. The following lemma, proven in Appendix~\ref{app:hv:type1}, presents some useful properties of these sets.
...
\extraspacing {\underline{\em Example 4.6.}} It is clear from Figure~\ref{fig:hv:type1:ex}d that $\cross_H(\alpha^*)$ contains $h_2, h_4$, and $h_5$, sorted in ascending order of their y-coordinates. Recall that $\gbot(\bm{t}_1) = \gbot(\bm{t}_2) = \alpha'$. Both $\dcross_H(\bm{t}_1) = \set{h_2}$ and $\dcross_H(\bm{t}_2) = \set{h_2, h_4, h_5}$ are indeed prefixes of the sorted $\cross_H(\alpha^*)$, as stated in Lemma~\ref{lmm:hv:type1:cross-r*}.
\done
...
\myeqn{
    \J_2  &=& \{(r_1, ..., r_{k-2}, h, v) \in \J(R_1, ..., R_{k-2}, H, V) \mid \text{$r_{k-2}$ covers the left endpoint of $h$}\}. \label{eqn:hv:type2:J2}
}
...
\extraspacing {\underline{\em Example 5.1.}} To illustrate our algorithm, we will utilize the running example in Figure~\ref{fig:hv:type2:ex}a, where $k = 4$, and $R_1 = \set{\alpha}$, $R_2 = \set{\beta_1, \beta_2, \beta_3}$, $H$ $=$ $\set{h_1,$ $h_2,$ $h_3}$, and $V = \set{v_1, v_2}$. The set $\J_2$ contains the following tuples: $(\alpha, \beta_1, h_2, v_1)$, $(\alpha, \beta_1, h_3, v_1)$, $(\alpha, \beta_2, h_3, v_1)$, $(\alpha, \beta_3, h_3, v_1)$, and $(\alpha, \beta_3, h_3, v_2)$. \done
...
Construct
\myeqn{
    H' &=& \{h' \mid h \in H \text{ and its trimmed segment $h'$ exists} \}. \label{eqn:hv:type2:H'}
}
...
\extraspacing {\bf Generating $\bm{\J_2}$.} Take any $(k-1)$-tuple $\bm{t} = (r_1, ..., r_{k-3}, h', v) \in \J(R_1, ..., R_{k-3}, H', V)$. Note that $B_\bm{t}$ --- defined in \eqref{eqn:B_t} --- is the point $h' \cap v$ (the intersection of $h'$ and $v$). Suppose that $h' = [x_1, x_2] \times y$ and $B_\bm{t} = (x, y)$; we define the {\em effective horizontal segment} of $\bm{t}$ as the horizontal segment $[x_1, x] \times y$. This allows us to define
\myeqn{
    \contain_{R_{k-2}}(\bm{t}) = \{r \in R_{k-2} \mid \text{$r$ contains the effective horizontal segment of $\bm{t}$}   \}
    \label{eqn:type2:contained-t}
}
The above should not be confused with \eqref{eqn:contained}, where the ``$\contain$'' function takes a segment as the parameter, rather than a tuple.
...
$\contain_{R_{k-2}}(\bm{t}_1) = \contain_{R_2}(\bm{t}_1) = \set{\beta_1}$, $\contain_{R_2}(\bm{t}_2) = \set{\beta_1, \beta_2, \beta_3}$, and $\contain_{R_2}(\bm{t}_3) = \set{\beta_3}$.
\done

\vgap 

We prove the next lemma in Appendix~\ref{app:hv:type2} (the reader may want to be reminded that, for each $\bm{t} \in \J(R_1, ..., R_{k-3}, H', V)$, $\bm{t}[k-2]$ is a horizontal segment and $\bm{t}[k-1]$ is a vertical segment).

\begin{lemma} \label{lmm:hv:type2:properties}
    All the following statements are true:
    \myenums{
        \item Consider any $(k-1)$-tuple $\bm{t} \in \J(R_1, ..., R_{k-3}, H', V)$. Denote by $h$ the full segment of $\bm{t}[k-2]$. Then, for any $r \in \contain_{R_{k-2}}(\bm{t})$, the $k$-tuple $(\bm{t}[1], ..., \bm{t}[k-3], r, h, \bm{t}[k-1])$ belongs to $\J_2$.
        
        \item Consider any $k$-tuple $(r_1, ..., r_{k-2}, h, v) \in \J_2$. Let $h'$ be the trimmed segment of $h$ and set $\bm{t} = (r_1, ..., r_{k-3}, h', v)$. Then, $\bm{t} \in \J(R_1, ..., R_{k-3}, H', V)$ and $r_{k-2} \in \contain_{R_{k-2}}(\bm{t})$.
        
        \item $\sum_{\bm{t}} |\contain_{R_{k-2}}(\bm{t})| \le \out$, where the summation is over all $\bm{t} \in \J(R_1, ..., R_{k-3}, H', V)$.
    }
\end{lemma}
...
\mytab{
    ...
    \> 4.\>\> {\bf for} each $r \in \contain_{R_{k-2}}(\bm{t})$ {\bf do} \\
    \> 5.\>\>\> add $(\bm{t}[1], ..., \bm{t}[k-3], r, h, \bm{t}[k-1])$ to $\J_2$
}

\vgap

The correctness of the algorithm follows from statements (1) and (2) of Lemma~\ref{lmm:hv:type2:properties}. Furthermore, if we are given $\contain_{R_{k-2}}(\bm{t})$ for each $\bm{t}$, statement (3) of Lemma~\ref{lmm:hv:type2:properties}
assures us that the algorithm runs in $O(1 + k \cdot |\J(R_1, ..., R_{k-3}, H', V)| + k \sum_\bm{t} |\contain_{R_{k-2}}(\bm{t})|) = O(1 + k \cdot \out)$ time, where the derivation used Lemma~\ref{lmm:hv:type2:recur-output} and statement (3) of Lemma~\ref{lmm:hv:type2:properties}.

\extraspacing {\underline{\em Example 5.4.}} For $\bm{t}_1 = (\alpha, h_2', v_1)$, the full segment of $h_2'$ is $h_2$. As $\beta_1$ is the only rectangle in $\contain_{R_2}(\bm{t}_1)$, Line 5 of the algorithm adds tuple $(\alpha, \beta_1, h_2, v_1)$ to $\J_2$. For $\bm{t}_2 = (\alpha, h_3', v_1)$, the full segment of $h_3'$ is $h_3$. As $\contain_{R_2}(\bm{t}_2) = \set{\beta_1, \beta_2, \beta_3}$, Line 5 adds $(\alpha, \beta_1, h_3, v_1)$, $(\alpha, \beta_2, h_3, v_1)$, and $(\alpha, \beta_3, h_3, v_1)$ to $\J_2$. Finally, the processing of $\bm{t}_3 = (\alpha, h_3', v_2)$ adds $(\alpha, \beta_3, h_3, v_2)$ to $\J_2$.
\done
...
\extraspacing {\underline{\em Example 5.5.}} As mentioned, $\J(R_1, H', V)$ has 3 tuples $\bm{t}_1$, $\bm{t}_2$, and $\bm{t}_3$. Both $\bm{t}_2 = (\alpha, h_3', v_1)$ and $\bm{t}_3 = (\alpha, h_3', v_2)$ have $h_3'$ as the horizontal segment. Therefore, $\minleft(h_3')$ equals 5, which is the smaller between the x-coordinate of $v_1$ and that of $v_2$. The minimal segment $h_3^*$ of $h_3'$ is shown in Figure~\ref{fig:hv:type2:ex}(c). On the other hand, it is easy to verify that $\minleft(h_2')$ is the x-coordinate of $v_1$. The minimal segment $h_2^*$ of $h_2'$ is also shown in Figure~\ref{fig:hv:type2:ex}(c).
\done
...
We are interested in the sets $\contain_{R_{k-2}}(h^*)$ of the segments $h^*$ in $H^*$, where $\contain_{R_{k-2}}(h^*)$ --- defined in \eqref{eqn:contained} --- is the set of rectangles in $R_{k-2}$ containing $h^*$. These sets have some useful properties:

\begin{lemma} \label{lmm:hv:type2:contained-h*}
    Both statements below are true: 
    \myenums{
        \item $\sum_{h^* \in H^*} |\contain_{R_{k-2}}(h^*)| \le \out$.
        
        \item Consider any tuple $\bm{t} \in \J(R_1, ..., R_{k-3}, H', V)$. Set $h' = \bm{t}[k-2]$ and let $h^*$ be the minimal segment of $h'$. Then, $$\contain_{R_{k-2}}(\bm{t}) \subseteq \contain_{R_{k-2}}(h^*).$$ Furthermore, if the rectangles $r$ in $\contain_{R_{k-2}}(h^*)$ are sorted in descending order of $\xright(r)$, then $\contain_{R_{k-2}}(\bm{t})$ includes a prefix of the sorted order.
    }
\end{lemma}

The proof can be found in Appendix~\ref{app:hv:type2}.

\extraspacing {\underline{\em Example 5.6.}} It is clear from Figure~\ref{fig:hv:type2:ex}(d) that $\contain_{R_2}(h_3^*)$ has rectangles $\beta_3, \beta_2$, $\beta_1$, sorted in descending order of their right boundaries' x-coordinates. Consider $\bm{t}_2 = (\alpha, h_3', v_1)$ and $\bm{t}_3 = (\alpha, h_3', v_2)$. Segment $h_3^*$ is the minimal segment of $h_3'$. As stated in Lemma~\ref{lmm:hv:type2:contained-h*}, both $\contain_{R_2}(\bm{t}_2) = \set{\beta_1, \beta_2, \beta_3}$ and $\contain_{R_2}(\bm{t}_3) = \set{\beta_1}$ are prefixes of the sorted order of $\contain_{R_2}(h_3^*)$. Regarding $h_2^*$, it is the minimal segment of $h_2'$, and $\contain_{R_2}(h_2^*)$ contains only  $\beta_1$. For $\bm{t}_1 = (\alpha, h_2', v_1)$, $\contain_{R_2}(\bm{t}_1) = \set{\beta_1}$ is a (trivial) prefix of $\contain_{R_2}(h_2^*)$, as is also consistent with the lemma.
\done

Finding the $\contain_{R_{k-2}}(h^*)$ sets of all $h^* \in H^*$ is an instance of Problem $\mathscr{E}$ (with $H^*$ and $R_{k-2}$ as the input). The cost is $O(n \log n + \out)$ according statement (1) of Lemma~\ref{lmm:hv:type1:cross-r*} and the discussion in Section~\ref{sec:bricks}. Note that, for each $\contain_{R_{k-2}}(h^*)$ computed, the rectangles $r$ therein have been sorted in descending order of $\xright(r)$.


\extraspacing {\bf Computing the ``$\bm{\contain_{R_{k-2}}(t)}$'' Sets.} Statement (2) of Lemma~\ref{lmm:hv:type2:contained-h*} allows us to produce $\contain_{R_{k-2}}(\bm{t})$ --- defined in \eqref{eqn:type2:contained-t} --- for each $\bm{t} \in \J(R_1, ..., R_{k-3}, H', V)$ as follows. First, fetch the (already computed) minimal segment $h^*$ of $\bm{t}[k-2]$ in $O(1)$ time. Then, scan the rectangles $r$ of $\contain_{R_{k-2}}(h^*)$ in descending order of $\xright(r)$. For each $r$ scanned, check whether $r \in \contain_{R_{k-2}}(\bm{t})$, or equivalently, whether $r$ covers $B_\bm{t}$ (recall that $B_\bm{t}$ is a point); the cost of this inspection is $O(1)$. Abort the scan as soon as $r \notin
\contain_{R_{k-2}}(\bm{t})$. This way, $\contain_{R_{k-2}}(t)$ can be decided in $O(k + |\contain_{R_{k-2}}(\bm{t})|)$ time. Doing so for all $\bm{t} \in \J(R_1, ..., R_{k-3},$ $H', V)$ takes $O(k \cdot |\J| + \sum_\bm{t} |\contain_{R_{k-2}}(\bm{t})|) = O(k \cdot \out)$ time, where the derivation used Lemma~\ref{lmm:hv:type2:recur-output} and statement (3) of Lemma~\ref{lmm:hv:type2:properties}.
...
Let $P'$ be the set of remaining points in $P$ after the aforementioned removal. Next, for each $j \in [k-1]$, solve the reporting version of Problem $\mathscr{A}$ by feeding $P'$ and $R_j$ as the input. This produces the set $\contain_{R_j}(p)$ for each point $p \in P'$, where $\contain_{R_j}(p)$ is defined in \eqref{eqn:contained} (treating $p$ as a degenerated ``horizontal segment'') and includes all rectangles of $R_j$ covering $p$. By the discussion in Section~\ref{sec:bricks}, the total cost of this step is bounded by
\myeqn{
    O\Big(kn \log n + \sum_{p \in P'} \sum_{j \in [k-1]} |\contain_{R_j}(p)| \Big). \label{eqn:ksj:cat1:help1}
}
...
\myeqn{
    (r_1, ..., r_{k-1}) \in \contain_{R_1}(p) \times ... \times \contain_{R_{k-1}}(p) \nn
    %\label{eqn:ksj:cat1:help2}
}
we add $(r_1, ..., r_{k-1}, r)$ to $\J^\cat_k$. Performing the above for all $p \in P'$ generates the whole $\J^\cat_k$ in $O(1 + k \cdot|\J^\cat_k|)$ time. The way $P'$ is computed ensures that $\contain_{R_j}(p) \ne \emptyset$ for each $j \in [k-1]$. Hence, $\sum_{j \in [k-1]} |\contain_{R_j}(p)| \le \prod_{j \in [k-1]} |\contain_{R_j}(p)|$, which implies that \eqref{eqn:ksj:cat1:help1} is bounded by $O(kn \log n + k \cdot |\J^\cat_k|) = O(kn \log n + k \cdot \out)$ .
...
Performing the above for all $i, j \in [k]$ with $i \ne j$ leads to a total time complexity of $O(k^3) \cdot (F_{k-1}(n, \out) + n \log n + k \cdot \out)$. A category-2 result tuple $\bm{t}$ may be seen more than once (this can happen if, for example, more than one rectangle in $\bm{t}$ has the same top-edge). Again, duplicate removal can be achieved at no extra cost asymptotically.
...
%
% Fix an integer $k \ge 3$ and $k$ integers $n_1, n_2, ..., n_k$ at least 1. Define $n = \sum_{i=1}^k n_i$. An {\em $(n_1, n_2, ..., n_k)$-input} to the $k$-SJ problem comprises $k$ sets of rectangles $R_1, R_2, ..., R_k$ in $\real^2$ such that $|R_i| = n_i$ for each $i \in [k]$. We assume that each rectangle in $R_i$ ($i \in [k]$) is given a distinct ID from $[n_i]$.
%
% \vgap
%
...
% }
% At a leaf, we look at its associated result and, for each $(\lambda_1, \lambda_2, ..., \lambda_k)$ in that result, output $(r_1, r_2, ..., r_k)$ where $r_i$ ($i \in [k]$) is the rectangle with ID $\lambda_i$ in $R_i$. The algorithm is required to produce the correct output on all $(n_1, n_2, ..., n_k)$-inputs. Its {\em worst-case cost} is the height of $\T$.
%
%
% \vgap
%
...
\extraspacing {\bf Terminology.} Each point $(x, y)$ is said to {\em define} a y-coordinate $y$, a horizontal segment $[x_1, x_2] \times y$ is said to {\em define} a y-coordinate $y$, and each rectangle $r = [x_1, x_2] \times [y_1, y_2]$ is said to {\em define} two y-coordinates $y_1$ and $y_2$. These definitions permit us to conveniently specify a set of y-coordinates using expressions like ``the set of four y-coordinates defined by point $p$, horizontal segment $h$, and rectangle $r$''.
...
Next, we aim to produce, for each pair $(h, r) \in H \times r$ such that $h$ crosses $r$, a pair $(h, \lambda)$ where $\lambda$ is the ID of $r$. These pairs may be output in an arbitrary order. Sort the set of y-coordinates defined by all the horizontal segments and rectangles. Sweep a horizontal line $\ell$ from $y = -\infty$ to $y = \infty$. At all times, we maintain the set $R_\ell$ of rectangles in $R$ that intersect with $\ell$. Let $S_\ell$ be the set of x-ranges of the rectangles in $R_\ell$; we store $S_\ell$ in a PST $\T$. Specifically, when $\ell$ hits the bottom (resp., top) edge of a rectangle $r = [x_1, x_2] \times [y_1, y_2]$ of $R$, we insert (resp., delete) $[x_1, x_2]$ into (resp., from) $\T$, which can be done in $O(\log n)$ time. We call the rectangle $r$ the {\em host} of $[x_1, x_2]$ and store its ID together with $[x_1, x_2]$ in $\T$. When $\ell$ hits a horizontal segment $h = [z_1, z_2] \times y$ of $H$, perform a reverse-containment query on $\T$ to find all the intervals in $S_\ell$ that contain $[z_1, z_2]$; the query cost is $O(\log n + K_h)$, where $K_h$ is the number of intervals reported. For each retrieved interval $[x_1, x_2]$, we also obtain the ID $\lambda$ of its host rectangle $r$, and output the pair $(h, \lambda)$.

\vgap

Each rectangle of $R$ necessitates one insertion and one deletion in $\T$. All these insertions and deletions take $O(n \log n)$ time in total. Each segment $h$ of $H$ performs a reverse-containment query on $\T$, which has a cost of $O(\log n + K_h)$. All these queries demand a total cost of $O(n \log n + \sum_h K_h)$. The sum $\sum_h K_h$ here is equal to the total size of $\contain_R(h)$ for all the $h \in H$. The total size is equivalent to $\out$. The cost so far is therefore $O(n \log n + \out)$.

\vgap

Let $L$ be the list of $(h, \lambda)$ pairs produced (the size of $L$ is $\out$). We now proceed to sort $L$ in ascending order of the $\lambda$-field (which is a rectangle ID), breaking ties arbitrarily. Because the IDs are integers created by the algorithm, we are permitted to sort $L$ using {\em counting sort} without violating the comparison-based requirements.
%(see Appendix~\ref{app:comp-class}).
The counting sort finishes in $O(|R| + |L|) = O(n + \out)$ time, recalling that all the IDs are in $[|R|]$.

\vgap

Finally, we generate, for each $h$, its set $\contain_R(h)$ (i.e., the set of rectangles covering $h$), where the rectangles $r$ are sorted by $\xright(r)$. To start with, initialize an empty linked list for every $h \in H$. Inspect the pairs $(h, \lambda) \in L$ in ascending order of the ID-field $\lambda$. For each pair $(h, \lambda)$ examined, identify the rectangle $r$ whose ID is $\lambda$ and add $r$ to the linked list of $h$. By the way the rectangle IDs were generated, it is clear that $\xright(r)$ is larger than or equal to the x-coordinates of the right edges of the rectangles already in the linked list. The whole scan over $L$ finishes in $O(1 + \out)$ time, and produces the correct output for Problem $\mathscr{E}$.


\section{Supplementary Proofs for Section~\ref{sec:hv:type1}} \label{app:hv:type1}

%This section supplies the proofs postponed from Section~\ref{sec:hv:type1}. 

We start by presenting two properties underneath the procedures designed in Section~\ref{sec:hv:type1}. These properties will enable us to construct simpler proofs later.
...
\begin{proposition} \label{prop:app:hv:type1:help1}
    ...
    \myitems{
        \item If a horizontal segment $h \in H$ crosses $r$, then $h$ must also cross $r'$.
        \item If a vertical segment $v \in V$ crosses $r$, then $v$ must also cross $r'$.
    }
\end{proposition}
...
We now proceed to elaborate the proofs postponed from Section~\ref{sec:hv:type1}. The order of the subsequent proofs will not strictly follow the sequence in which they are referenced in Section~\ref{sec:hv:type1}. In particular, we will prove Lemma~\ref{lmm:hv:type1:properties} before Lemma~\ref{lmm:hv:type1:recur-output}, because the claims of the former lemma can be used to produce a succinct argument for the latter.

\extraspacing {\bf Proof of Lemma~\ref{lmm:hv:type1:properties}.} We will prove each statement in turn.

\extraspacing \underline{\em Proof of Statement (1).} Take any $(k-2)$-tuple $\bm{t} \in \J(R_1', R'_2, ..., R'_{k-2})$. Let $r_i$ ($i$ $\in$  $[k-2]$) be the full rectangle of $\bm{t}[i]$. Fix any $h \in \dcross_H(\bm{t})$ and any $v \in \dcross_V(\bm{t})$. The segments $h$ and $v$ both cross $B_\bm{t}$, as can be seen directly from the definitions of $\dcross_H(\bm{t})$ and $\dcross_V(\bm{t})$. Thus, $h \cap v$ is a point in $B_\bm{t} = \bigcap_{i=1}^{k-2} \bm{t}[i]$. As $r_i$ ($i \in [k-2]$) is the full rectangle of $\bm{t}[i]$, we know that $\bigcap_{i=1}^{k-2} r_i$ covers $B_\bm{t} = \bigcap_{i=1}^{k-2} \bm{t}[i]$. Hence, point $h \cap v$ falls in $\bigcap_{i=1}^{k-2} r_i$, indicating that $h\cap v \cap \bigcap_{i = 1}^{k-2}r_i \neq \emptyset$. It follows that $(r_1,...,r_{k-2},h,v)$ is a result tuple in $\J(R_1,...,R_{k-2},H,V)$.

\extraspacing \underline{\em Proof of Statement (2).} Take any $k$-tuple $(r_1,...,r_{k-2},h,v) \in \J_1$. By definition of $\J_1$, segments $h$ and $v$ cross each of the rectangles $r_1,..., r_{k-2}$. By Proposition~\ref{prop:app:hv:type1:help1}, segments $h$ and $v$ must also cross each of the trimmed rectangles $r_1',..., r_{k-2}'$. Hence, $B_\bm{t} = \bigcap_{i = 1}^{k-2}r_i'$ is non-empty as it contains the point $h \cap v$, which proves the first claim $\bm{t} = (r_1', ..., r_{k-2}') \in \J(R_1',...,R_{k-2}')$.

\vgap 

Next, we prove the second claim, i.e., $h \in \dcross_H(\bm{t})$ and $v \in \dcross_V(\bm{t})$. It suffices to show only the former due to symmetry. For that purpose, we need to argue that $h$ crosses $\gbot(\bm{t})$ and $B_\bm{t}$. The first part, $h$ crossing $\gbot(\bm{t})$, is done because as mentioned $h$ crosses each of $r_1', ..., r_{k-2}'$, and $\gbot(\bm{t})$ is merely one of those rectangles. To prove that $h$ crosses $B_\bm{t}$, first note that $h \cap B_\bm{t} \ne \emptyset$ because as explained before $B_\bm{t}$ contains $h \cap v$. On the other hand, as $\gbot(\bm{t})$ covers $B_\bm{t}$, the fact of $h$ crossing $\gbot(\bm{t})$ indicates that the x-range of $h$ contains that of $B_\bm{t}$. Combining this with $h \cap B_\bm{t} \ne \emptyset$ shows that $h$ crosses $B_\bm{t}$.

\extraspacing \underline{\em Proof of Statement (3).} We will prove 
\myeqn{
    \sum_{\bm{t}\in \J(R_1',...,R_{k-2}')} |\dcross_H(\bm{t})| \cdot |\dcross_V(\bm{t})| \leq \out \label{eqn:hv:type1:properties:proof-help1}
} 
which implies statement (3) because, by Proposition~\ref{prop:app:hv:type1:help2}, $|\dcross_H(\bm{t})| \ge 1$ and $|\dcross_V(\bm{t})| \ge 1$  for any $\bm{t}$ in the summation. Our proof resorts to the algorithm generate-$\J^*$ given in Section~\ref{sec:hv:type1}. This algorithm adds to $\J^*$ as many tuples as calculated by the left hand side of \eqref{eqn:hv:type1:properties:proof-help1}. By statement (1) of Lemma~\ref{lmm:hv:type1:properties}, the $\J^*$ produced must be a subset of $\J(R_1,...,R_{k-2},H,V)$, whereas $\out = |\J(R_1,...,R_{k-2},H,V)|$. This establishes the inequality in \eqref{eqn:hv:type1:properties:proof-help1}.

\extraspacing \noindent {\bf Proof of Lemma~\ref{lmm:hv:type1:recur-output}.} Our proof again resorts to the algorithm generate-$\J^*$ in Section~\ref{sec:hv:type1}. By Proposition~\ref{prop:app:hv:type1:help2}, both $\dcross_H(\bm{t})$ and $\dcross_V(\bm{t})$ are non-empty for any $\bm{t} \in \J(R_1',$ $...,R_{k-2}')$. Therefore, in processing this $\bm{t}$, the algorithm adds at least one new tuple to $\J^*$. Hence, $|\J(R_1',...,R_{k-2}')| \le |\J^*|\le |\J(R_1,...,R_{k-2},H,V)| = \out$, where the second inequality used Statement (1) of Lemma~\ref{lmm:hv:type1:properties}.

\extraspacing {\bf Computing $\bm{R^*_1, R^*_2, ..., R^*_{k-2}}$.} We consider, w.l.o.g., that each rectangle in the input $R_1 \cup ... \cup R_{k-2}$ is given a distinct integer ID in $[n]$. This allows us to create an array of size $n$ and allocate an array cell to each $r \in R_1 \cup ... \cup R_{k-2}$. The cell can be accessed by the ID of $r$ in constant time.
...
\extraspacing \underline{\em Proof of Statement (1).} We will map each $r^*$ of $\bigcup_{i = 1}^{k-2} R_i^*$ to a unique tuple $\bm{t}$ in $\J(R_1',...,R_{k-2}')$ satisfying $\cross_H(r^*) \subseteq \dcross_H(\bm{t})$. The mapping allows us to derive
\myeqn{
...
The mapping is as follows. Consider an arbitrary $r^*\in \bigcup_{i = 1}^{k-2} R_i^*$. Recall that $r^*$ is the top-sliced of some rectangle $r'$. Specifically, if $r' = [x_1,x_2]\times [y_1,y_2]$, then $r^* = [x_1,x_2]\times [y_1,\maxtop(r')]$. By the definition of $\maxtop(r')$ in \eqref{eqn:hv:type1:topsliced}, there exists a tuple $\bm{t}\in \J(R_1',...,R_{k-2}')$ satisfying $\gbot(\bm{t}) = r'$ and $\maxtop(r') = \ytop(B_{\bm{t}})$. We map $r^*$ to $\bm{t}$.

\vgap

Next, we will prove 
\begin{center}
    {\bf Claim 1:} If a segment $h \in \cross_H(r^*)$, then $h \in \dcross_H(\bm{t})$.
\end{center}
For this purpose, we need to show that $h$ crosses both $\gbot(\bm{t}) = r'$ and $B_\bm{t}$.
\myitems {
    \item First, $h$ crosses $r'$ follows from the facts that (i) $h$ crosses $r^*$, (ii) $r^* \subseteq r'$, and (iii) $r^*$ and $r'$ share the same x-range. 
    
    %. Because $r^* \subseteq r'$, the fact of $h$ crossing $r^*$ tells us that $h \cap r' \ne \emptyset$. On the other hand, the fact of $h$ crossing $r^*$ also tells us that $\xleft(h) < x_1 \le x_2 < \xright(h)$ (recall that $[x_1, x_2]$ is the x-range of $r^*$). As $[x_1, x_2]$ is also the x-range of $r'$, the fact of $h \cap r' \ne \emptyset$ indicates that $h$ must cross $r'$.

    \vgap

    \item Then, we explain why $h$ crosses $B_{\bm{t}}$. Because $B_{\bm{t}}\subseteq r'$ and $h$ crosses $r'$ (just proved), no endpoint of $h$ can fall in $B_\bm{t}$. Let us take the y-coordinate $y_h$ of $h$. We will prove that the point $p = (\xleft(B_{\bm{t}}), y_h)$ is on the segment $h$ and is also in $B_{\bm{t}}$, suggesting $h\cap B_{\bm{t}} \neq \emptyset$. Once this is done, we can assert that $h$ crosses $B_{\bm{t}}$ (as no endpoint of $h$ falls in $B_\bm{t}$).
    
    \vgap

    \myitems{
        \item As $\gbot(\bm{t}) = r'$, the bottom edge of $B_{\bm{t}}$ is contained in $r'$. Thus, $\xleft(B_{\bm{t}})\in [x_1,x_2]$. The fact of $h$ crossing $r^*$ tells us that $\xleft(h) < x_1 \le x_2 < \xright(h)$, which leads to $\xleft(B_{\bm{t}}) \in [\xleft(h), \xright(h)]$, suggesting that $p \in h$.
        
        \vgap

        \item As $\gbot(\bm{t}) = r'$ and $\maxtop(r') = \ytop(B_\bm{t})$, the y-range of $B_\bm{t}$ is $[y_1, \maxtop(r')]$. Because $h \cap r^* \ne \emptyset$ (as $h$ crosses $r^*$) and $[y_1, \maxtop(r')]$ is also the y-range of $r^*$, we know that $y_h \in [y_1, \maxtop(r')]$, suggesting that $p \in B_{\bm{t}}$.
    }
}

% We now turn to the ``if direction'': assuming $h \in \dcross_H(\bm{t})$, we aim to show $h \in \cross_H(r^*)$. Recall that $r^* = [x_1,x_2] \times [y_1, \maxtop(r')]$. To prove that $h$ crosses $r^*$, we will argue that $\xleft(h) < x_1 \le x_2 < \xright(h)$ and $y_h \in [y_1, \maxtop(r')]$, where $y_h$ is the y-coordinate of $h$.
% \myitems{
%     \item The fact $h \in \dcross_H(\bm{t})$ tells us that $h$ crosses $r'$. As $[x_1, x_2]$ is the x-range of $r'$, we have $\xleft(h) < x_1 \le x_2 < \xright(h)$.
% 
%     \vgap
% 
%     \item The fact $h \in \dcross_H(\bm{t})$ also tells us that $h$ crosses $B_{\bm{t}}$. As $[y_1, \maxtop(r')]$ is the y-range of $B_\bm{t}$ (because $\gbot(\bm{t}) = r'$ and $\maxtop(r') = \ytop(B_\bm{t})$), we have $y_h \in [y_1, \maxtop(r')]$.
% }
...
\extraspacing \underline{\em Proof of Statement (2).} Take any tuple $\bm{t} \in \J(R'_1, ..., R'_{k-2})$. Set $r' = \gbot(\bm{t})$, and let $r^*$ be the top-sliced rectangle of $r'$. If we represent $r'$ as $[x_1,x_2] \times [y_1, y_2]$, then $r^*$ can be written as $[x_1, x_2] \times [y_1, \maxtop(r')]$.

\vgap

We will first prove $\dcross_H(\bm{t})\subseteq \cross_H(r^*)$ or equivalently: if a segment $h \in \dcross_H(\bm{t})$, then $h$ crosses $r^*$. To do so, we need to explain why $\xleft(h) < x_1 \le x_2 < \xright(h)$ and $y_h \in [y_1,$ $\maxtop(r')]$.
\begin{itemize}
    \item The fact of $h \in \dcross_H(\bm{t})$ tells us that $h$ crosses $\gbot(\bm{t})$, which is $r'$. As the x-range of $r'$ is $[x_1,x_2]$, it must hold true that $\xleft(h) < x_1 \le x_2 < \xright(h)$.

    \vgap

    \item The fact of $h \in \dcross_H(\bm{t})$ also tells us that $h$ crosses $B_{\bm{t}}$. Hence, $y_h \in [\ybot(B_\bm{t}), \ytop(B_{\bm{t}})]$. From $r' = \gbot(\bm{t})$, we get $\ybot(B_\bm{t}) = \ybot(r') = y_1$. By the definition of $\maxtop(r')$ in \eqref{eqn:hv:type1:maxtop}, we know $\ytop(B_{\bm{t}}) \leq \maxtop(r')$. It thus follows that $y_h \in [y_1, \maxtop(r')]$.
\end{itemize}

Next, assuming that the segments of $\cross_H(r^*)$ are sorted in ascending order of their y-coordinates,
we will prove that $\dcross_H(\bm{t})$ includes a prefix of the sorted order. It suffices to establish the following equivalent claim:

\vgap

\minipg{0.8\linewidth} {
    {\bf Claim 2:} If a segment $h \in \cross_H(r^*)$ has y-coordinate $y_h \le \ytop(B_{\bm{t}})$, then $h$ must be in $\dcross_H(\bm{t})$.
}
...
\item We first show that $h$ crosses $r'$, or equivalently: $\xleft(h)  < x_1 \leq x_2 < \xright(h)$ and $y_h \in [y_1, y_2]$. These conditions hold true because (i) $h$ crosses $r^* = [x_1,x_2]\times [y_1, \maxtop(r')]$, and (ii) the definition of a top-sliced rectangle in \eqref{eqn:hv:type1:topsliced} tells us $\maxtop(r')\leq y_2$.
...
We start by presenting two properties underneath the procedures designed in Section~\ref{sec:hv:type2}. These properties will enable us to construct simpler proofs later.

\begin{proposition} \label{prop:app:hv:type2:help1}
    Consider any $k$-tuple $(r_1, ..., r_{k-2}, h, v) \in \J_2$. Define $h'$ as the trimmed segment of $h$. It holds that $h \cap v = h' \cap v$.
\end{proposition}

\begin{proof}
    Set $p = h \cap v$. As $h'$ is contained in $h$, for proving $p = h' \cap v$, it suffices to explain why $p$ is on $h'$. Let us represent $h$ as $[x_1,x_2] \times y$. Accordingly, the segment $h'$ can be written as $[x_1, x_2'] \times y$ for some $x_2' \in [x_1, x_2]$. By the definition of a trimmed segment, $(x_2',y)$ is the rightmost point on $h$ that falls in a left-end covering rectangle of $h$ in $R_{k-2}$. As $(r_1,...,r_{k-2},h,v) \in \J_2$, we know that $r_{k-2}$ is a left-end covering rectangle of $h$, and $r_{k-2}$ covers $p$ (which is a point on $h$). Therefore, the x-coordinate of $p$ cannot exceed $x_2'$, allowing us to assert that $p$ is on $h'$.
\end{proof}

\begin{proposition} \label{prop:app:hv:type2:help2}
    For each tuple $\bm{t} \in \J(R_1,...,R_{k-3},H',V)$ (where $H'$ is defined in \eqref{eqn:hv:type2:H'}), the set $\contain_{R_{k-2}}(\bm{t})$ is non-empty.
\end{proposition}

\begin{proof}
    Set $h' = \bm{t}[k-2]$. By the definition of a trimmed segment, there exists a rectangle $r_{k-2} \in R_{k-2}$ covering both endpoints of $h'$. Thus, the effective horizontal segment of $\bm{t}$ (defined in Section~\ref{sec:hv:type2}) --- which must be contained in $h'$ --- must also be covered by $r_{k-2}$. Hence, $\contain_{R_{k-2}}(\bm{t})$ contains $r_{k-2}$ and thus cannot be empty.
\end{proof}



We now proceed to elaborate the proofs postponed from Section~\ref{sec:hv:type2}. The order of the proofs here will not strictly follow the sequence in which they are referenced in Section~\ref{sec:hv:type2}. In particular, we will prove Lemma~\ref{lmm:hv:type2:properties} before Lemma~\ref{lmm:hv:type2:recur-output}, because the claims of the former lemma can be used to construct a simple argument for the latter.

\extraspacing {\bf Proof of Lemma~\ref{lmm:hv:type2:properties}.} We will prove each statement in turn.

\extraspacing \underline{\em Proof of Statement (1).} Take any tuple $\bm{t} = (r_1,...,r_{k-3}, h', v) \in \J(R_1,...,R_{k-3},H',V)$. The point $h' \cap v$ is covered by all of $r_1,...,r_{k-3}$. Let $h$ be the full segment of $h'$. As $h$ contains $h'$, we have $h \cap v = h' \cap v$.

\vgap

Consider any rectangle $r_{k-2} \in \contain_{R_{k-2}}(\bm{t})$. From the definition of $\contain_{R_{k-2}}(\bm{t})$, we know that $r_{k-2}$ covers the effective horizontal segment of $\bm{t}$ whose right endpoint is $h' \cap v$. This means that $h \cap v$ falls in $r_{k-2}$. We now have $h \, \cap \, v \, \cap \, \bigcap_{i = 1}^{k-2} r_i$ equals $h' \cap v$ and hence is non-empty, meaning that $(r_1,...,r_{k-3}, r_{k-2}, h, v) \in \J(R_1,...,R_{k-2},H,V)$.

\vgap

To show $(r_1,...,r_{k-3}, r_{k-2}, h, v) \in \J_2$, we still need to explain why $r_{k-2}$ covers the left endpoint of $h$. This follows immediately from the fact that $r_{k-2}$ covers the effective horizontal segment of $h'$ (which shares the same left endpoint as $h$).

\extraspacing \underline{\em Proof of Statement (2).} Take any $k$-tuple $(r_1, ..., r_{k-2}, h, v) \in \J_2$. Set $p = h \cap v$, and define $h'$ as the trimmed segment of $h$. We first argue that the $(k-1)$-tuple $\bm{t} = (r_1, ..., r_{k-3}, h', v)$ belongs to $\J(R_1,...,R_{k-3}, H', V)$, or equivalently, the point $h' \cap v$ falls in all of $r_1,...,r_{k-3}$. By Proposition~\ref{prop:app:hv:type2:help1}, $h' \cap v$ is the same point as $p$. From $(r_1,...,r_{k-2},h,v) \in \J(R_1,...,R_{k-2},H,V)$, we know that $p = h \cap v$ falls in all of $r_1,...,r_{k-2}$. It thus follows that $r_1,...,r_{k-3}$ all cover $h' \cap v$.

\vgap

Next, we argue that $r_{k-2}\in \contain_{R_{k-2}}(\bm{t})$. Let $p'$ be the left endpoint of $h$. Because$h' \cap v = h \cap v = p$, the effective horizontal segment of $\bm{t}$ is the segment $p'p$. To prove $r_{k-2} \in \contain_{R_{k-2}}(\bm{t})$, we need to explain why $r_{k-2}$ covers segment $p'p$. This is true because:

\begin{itemize}
    \item $r_{k-2}$ covers $p$, as we already know;
    \item $r_{k-2}$ covers $p'$, as it is a left-end covering rectangle of $h$.
\end{itemize}
...
\myeqn{
    \sum_{\bm{t} \in \J(R_1, ..., R_3, H', V)} |\contain_{R_{k-2}}(\bm{t})| \nn
}
...
\extraspacing \noindent {\bf Proof of Lemma~\ref{lmm:hv:type2:recur-output}.} Again, we resort to the algorithm generate-$\J_2$ . For each tuple $\bm{t} \in \J(R_1,...,$ $R_{k-3},$ $H',V)$, we know by Proposition~\ref{prop:app:hv:type2:help2} that $\contain_{R_{k-2}}(\bm{t}) \ne \emptyset$; thus, the algorithm adds at least one tuple to $\J_2$. Therefore, $|\J(R_1,...,R_{k-3},H',V)| \le |\J_2| \le \out$, where the derivation used the definition of $\J_2$; see \eqref{eqn:hv:type2:J2}.

\extraspacing {\bf Computing $\bm{H^*}$.} We assume, w.l.o.g., that each segment in the input $H$ is given a distinct integer ID in $[|H|]$. This allows us to create an array of size $|H| < n$ and allocate an array cell to each $h \in H$. The cell can be accessed by the ID of $h$ in constant time.
...
Finally, we construct $H^*$ by collecting the minimal segment (defined in \eqref{eqn:hv:type2:H*}) of every segment $h' \in H'$ with $\minleft(h') \neq \infty$. This step takes $O(|H'|) = O(n)$ time.

\extraspacing {\bf Proof of Lemma~\ref{lmm:hv:type2:contained-h*}.} We will prove each statement in turn.

\extraspacing \underline{\em Proof of Statement (1).} We will map each segment $h^* \in H^*$ to a unique tuple $\bm{t}\in \J(R_1,...,R_{k-3},H',V)$ satisfying $\contain_{R_{k-2}}(h^*) = \contain_{R_{k-2}}(\bm{t})$. The mapping allows us to derive
\myeqn{
    \sum_{h^* \in H^*} |\contain_{R_{k-2}}(h^*)|
    \le
    \sum_{\bm{t} \in \J(R_1,...,R_{k-3},H',V)} |\contain_{R_{k-2}}(\bm{t})|
    \leq \out \nn
}
where the last inequality used Statement (3) of Lemma~\ref{lmm:hv:type2:properties}.

\vgap

The mapping is as follows. Consider any $h^* \in H^*$. Recall that $h^*$ is the minimal segment of some segment $h' \in H'$. Specifically, if $h' = [x_1,x_2]\times y$, then $h^* = [x_1,\minleft(h')]\times y$. By the definition of $\minleft(h')$ in \eqref{eqn:hv:type2:H*}, there exists a tuple $\bm{t}\in \J(R_1,...,R_{k-3}, H', V)$ satisfying $\bm{t}[k-2] = h'$ and $\minleft(h') = \text{$x$-coordinate of the vertical segment $\bm{t}[k-1]$}$. We map $h^*$ to $\bm{t}$.

\vgap

Next, we will prove
\begin{center}
    {\bf Claim 1:} a rectangle $r \in \contain_{R_{k-2}}(h^*)$ if and only if $r \in \contain_{R_{k-2}}(\bm{t})$.
\end{center}
According to the definitions of $\contain_{R_{k-2}}(h^*)$ and $\contain_{R_{k-2}}(\bm{t})$ --- see \eqref{eqn:contained} and \eqref{eqn:type2:contained-t}, respectively --- it suffices to show that $h^*$ is the same as the effective horizontal segment of $\bm{t}$.

\vgap

Denote by $x$ the x-coordinate of $\bm{t}[k-1]$. Then, the point $h' \cap v$ can be written as $(x,y)$, where as mentioned $y$ is the y-coordinate of $h'$. The effective horizontal segment of $\bm{t}$ is therefore $[x_1,x]\times y$. Recall that $h^* = [x_1,\minleft(h')]\times y$, where (by definition of $\bm{t}$) $\minleft(h') = \text{$x$-coordinate of $\bm{t}[k-1]$} = x$. Therefore, $h^*$ is the same as the effective horizontal segment of $\bm{t}$, proving claim 1.

\vgap

It remains to show that no two distinct $h_1^*, h_2^* \in H^*$ can be mapped to the same tuple in $\J(R_1,...,R_{k-3}, H',V)$. Assume, on the contrary, that $h_1^*$ and $h_2^*$ are both mapped to $\bm{t}\in \J(R_1,...,R_{k-3},$ $H',V)$. Suppose that $h_1^*$ (resp., $h_2^*$) is the minimal segment of $h_1'$ (resp., $h_2'$). Under our mapping, it must hold that  $h_1' = h_2' = \bm{t}[k-2]$. However, the distinctness of $h_1^*$ and $h_2^*$ requires $h_1' \ne h_2'$, which yields a contradiction.

\extraspacing \underline{\em Proof of Statement (2).} Take any tuple $\bm{t} \in \J(R'_1, ..., R'_{k-3}, H', V)$. Set $h' = \bm{t}[k-2]$, and define $h^*$ as the minimal segment of $h'$. Let us write $h'$ as $[x_1,x_2] \times y$; accordingly, $h^*$ can be written as $[x_1, \minleft(h')] \times y$. Additionally, let $x$ be the x-coordinate of the vertical segment $\bm{t}[k-1]$. The effective horizontal segment of $\bm{t}$ can then be represented as $[x_1,x] \times y$.

\vgap

We will first prove $\contain_{R_{k-2}}(\bm{t})\subseteq \contain_{R_{k-2}}(h^*)$, or equivalently, every rectangle $r_{k-2} \in \contain_{R_{k-2}}(\bm{t})$ covers $h^*$. It suffices to show that $h^*$ is contained in the effective horizontal segment of $\bm{t}$, or equivalently, $\minleft(h') \le x$. By the definition in \eqref{eqn:minleft}, $\minleft(h')$ is the minimum x-coordinate of $\bm{t'}[k-1]$ among all $\bm{t'} \in  \J(R'_1, ..., R'_{k-3}, H', V)$ with $\bm{t'}[k-2] = h'$, Thus, $\minleft(h') \le x$ holds because $\bm{t}$ is merely one such $\bm{t'}$.

\vgap

Next, assuming that the rectangles $r \in \contain_{R_{k-2}}(h^*)$ have been sorted in descending order of $\xright(r)$, we will prove that $\contain_{R_{k-2}}(\bm{t})$ includes a prefix of the sorted order. It suffices to establish the following equivalent claim:

\vgap

\minipg{0.8\linewidth} {
    {\bf Claim 2:} If a rectangle $r \in \contain_{R_{k-2}}(h^*)$ satisfies the condition that $\xright(r) \ge x$ (where $x$ is the x-coordinate of $\bm{t}[k-1]$), then $r \in \contain_{R_{k-2}}(\bm{t})$.
}

\vgap

\noindent To prove the above, we must explain why the rectangle $r$ in the claim contains the effective horizontal segment of $\bm{t}$. Since $r \in \contain_{R_{k-2}}(h^*)$, we know that $r$ covers the segment $[x_1, \minleft(h')] \times y$. Hence, $[x_1, \minleft(h')]$ $\subseteq [\xleft(r), \xright(r)]$ and $y \in [\ybot(r), \ytop(r)]$. Using also the condition $\xright(r) \ge x$ given in claim 2, we can derive $[x_1, x] \subseteq [\xleft(r), \xright(r)]$. Therefore, $[x_1, x] \times y$, i.e. the effective horizontal segment of $\bm{t}$, is contained in $r$, establishing claim 2.
...
\myeqn{
    ...
    &=& c^2 \cdot k^3(k-1)^3 \cdot (F_{k-2}(n,\out) + n\log n + k\cdot \out) + c \cdot k^3 \cdot (n\log n + k\cdot \out ) \nn\\
    &<& c^2 \cdot k^3(k-1)^3 \cdot F_{k-2}(n,\out) +  (c+c^2)\cdot k^3(k-1)^3 \cdot (n\log n + k \cdot\out) \nn \\
    &\leq& c^3 \cdot k^3(k-1)^3(k-2)^3 \cdot F_{k-3}(n,\out) +
    (c+c^2+c^3)\cdot k^3(k-1)^3(k-2)^3 \cdot (n\log n + k \cdot\out) \nn\\
    &\leq ... \leq & %\nn \\
    c^{k-2} \cdot (k!)^3 \cdot F_{2}(n,\out) + \Big(\sum_{i =1}^{k-2}c^i \Big)\cdot (k!)^3 \cdot (n\log n + k \cdot\out) \nn \\
...
\myitems{
    ...
    \item Conversely, consider that $\J(R_1, R_2, R_3)$ is non-empty. Consider an arbitrary result tuple $(r_1, r_2, r_3) \in \J(R_1, R_2, R_3)$. Assume, w.l.o.g., that $r_1 = (-\infty, \infty) \times [u, u] \times [v, v]$, where $u < v$.
