...
Let $S$ be a set of segments all of which are either horizontal or vertical. Given a rectangle $r$, we define 
\myeqn{
...
Define $r' = [x_1, x_2] \times [y_1, y_2]$, where $x_1$ (resp., $x_2$) is x-coordinate of $v_1$ (resp., $v_2$), and $y_1$ (resp., $y_2$) is y-coordinate of $h_1$ (resp., $h_2$). We say that $r'$ is the {\em trimmed rectangle} of $r$, and conversely, $r$ is the {\em full rectangle} of $r'$. Note that $r'$ exists if and only if $r$ is crossed by at least one horizontal segment in $H$ and by at least one vertical segment in $V$. Moreover, it is possible for $h_1$ and $h_2$ to be the same segment; same for $v_1$ and $v_2$. 
...
Computing the ``segment $h_1$'' for each $r \in R_1$ is an instance of Problem $\mathscr{D}$ (the find-lowest version). By symmetry, so is the computation of $h_2, v_1$, and $v_2$ segments for each $r_1 \in R_1$. It thus follows from Section~\ref{sec:bricks} that $R_i'$ can be produced in $O(n \log n)$ time.

\vgap

We now solve the $(k-2)$-SJ problem on $R_1', R_2', ..., R_{k-2}'$ using the algorithm $\A$ supplied (see Lemma~\ref{lmm:hv}). This $(k-2)$-SJ clearly has an input size at most $n$, and let us represent its result as $\J(R_1', R_2, ..., R_{k-2})$. We prove in Appendix~\ref{app:hv}:
...
% }
\myeqn{
    \dcross_H(\bm{t}) = \set{h \in H \mid \text{$h$ crosses both $B_\bm{t}$ and $\gleft(\bm{t})$}} \label{eqn:dcross-H} \\
    \dcross_V(\bm{t}) = \set{h \in H \mid \text{$v$ crosses both $B_\bm{t}$ and $\gbot(\bm{t})$}}. \nn
    %\label{eqn:dcross-H} 
}
...
\begin{lemma} \label{lmm:hv:type1:properties}
    ...
    \myenums{
        ...
        \myenums{
            ...
            \item If the $k$-tuple $(r_1, r_2, ..., r_{k-2}, h, v)$ belongs to $\J_1$, then $h \in \dcross_H(\bm{t})$ and $v \in \dcross_V(\bm{t})$.
        }
...
Lemma~\ref{lmm:hv:type1:properties} assures us that the set $\J^*$ thus computed indeed satisfies \eqref{eqn:hv:type1:J*}. Furthermore, if we are given $\dcross_H(\bm{t})$ and $\dcross_V(\bm{t})$ for each $\bm{t}$, Lemma~\ref{lmm:hv:type1:properties} further asserts that the above algorithm runs in $O(1 + k \cdot |\J^*|) = O(1 + k \cdot \out)$ time.
...
\myeqn{
    \maxtop(r') &=& \max_{\bm{t} \in \J(R'_1, ..., R'_{k-2}): r' = \gbot(\bm{t})} \ytop(B_\bm{t}). 
}
The value $\maxtop(r')$ is set to $-\infty$, if no $\bm{t} \in \J(R'_1, ..., R'_{k-2})$ has $r'$ as the $\gbot(\bm{t})$. When $\maxtop(r') \ne -\infty$, assuming $r' = [x_1, x_2] \times [y_1, y_2]$, we introduce a rectangle
\myeqn{
...
We pay particular attention to the sets $\cross_H(r^*)$ of the rectangles $r^*$ in $R^*_i$, where $\cross_H(r^*)$, defined in \eqref{eqn:cross}, is the set of segments in $H$ crossing $r^*$. The following lemma, proven in Appendix~\ref{app:hv}, presents some crucial properties for these sets. 
...
\begin{lemma} \label{lmm:hv:type1:cross-r*}
    ...
    \myenums{
        ...
        \item Consider any $k$-tuple $\bm{t} \in \J(R'_1, ..., R'_{k-2})$. Let $r' = \gbot(\bm{t})$ and $r^*$ be the top-sliced rectangle of $r'$. Then, it must hold that $\dcross_H(\bm{t}) \subseteq \cross_H(r^*)$. Furthermore, if the (horizontal) segments of $\cross_H(r^*)$ have been sorted in ascending order of y-coordinate, then $\dcross_H(\bm{t})$ includes a prefix of the sorted order.
    }
\end{lemma}

\yf{example}

Finding the $\cross_H(r^*)$ sets of all $r^* \in R^*_i$ is an instance of the find-all-sorted version of Problem $\mathscr{D}$. Statement (1) of the above lemma assures us that the total time to do so for all $R^*_1, ..., R^*_{k-2}$ is bounded by $O(n \log n + \out)$. Note that, for each $\cross_H(r^*)$ computed, the (horizontal) segments therein has been sorted in ascending order of y-coordinate.



\extraspacing {\bf Computing the ``d-cross'' Sets.} We are ready explain how to compute $\dcross_H(\bm{t})$, defined in \eqref{eqn:dcross-H}, for any $\bm{t} \in \J(R'_1, ..., R'_{k-2})$. This, in fact, has been made easy by Statement (2) of Lemma~\ref{lmm:hv:type1:cross-r*}. First, identify the rectangle $r' = \gbot(\bm{t})$ and fetch its (already computed) top-sliced rectangle $r^*$. Second, compute $B_\bm{t}$ and $\gleft(\bm{t})$. These two steps require $O(k)$ time. Then, scan the segments in $\cross_H(r^*)$ in ascending order of their y-coordinates. For each segment $h$ scanned, check whether $h$ belongs to $\dcross_H(\bm{t})$, namely,  whether $h$ crosses both $B_\bm{t}$ and $\gleft(\bm{t})$; this can be done in constant time. Abort the scan as soon as $h \notin \dcross_H(\bm{t})$. This way, we produce $\dcross_H(\bm{t})$ in $O(k + |\dcross_H(\bm{t}|)$ time. Doing so for all $\bm{t} \in \J(R'_1, ..., R'_{k-2})$ takes $O(k |\J| + |\J^*|) = O(k \cdot \out)$ time.
