...
Figure~\ref{fig:type1}(b) shows an example with rectangles $r_1, r_2$, and $r_3$ (here $k = 5$). Their intersection is the gray rectangle $B$, which is bottom-bounded by $r_2$ and left-bounded by $r_3$. 
...
In Figure~\ref{fig:type1}(b), $H = \set{h_1, h_2, h_3}$ and $V = \set{v_1, v_2, v_3}$. As mentioned before, $r_a = r_2$ and $r_b = r_3$. We have $\cross(r_a, B; H) = \set{h_1, h_2}$ and $\cross(r_b, B; V) = \set{v_1, v_3}$. Note that $h_3$ is not in $\cross(r_a, B; H)$ because $h_3$ does not cross $B$, and $v_2$ is not in $\cross(r_b, B; V)$ because $v_2$ does not cross $r_3$.
