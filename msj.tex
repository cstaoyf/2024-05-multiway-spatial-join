...
\extraspacing {\bf Generating $\bm{\J^*}$.} Take an arbitrary $(k-2)$-tuple in $\bm{t} = (r'_1, r'_2, ...,$ $r'_{k-2}) \in \J(R_1', R'_2, ..., R'_{k-2})$. The reader should recall from Section~\ref{sec:bricks} that $B_\bm{t}$ is the intersection of of $r'_1, r'_2, ..., r'_{k-2}$, $\gleft(\bm{t})$ is the $r_i$ ($i \in [k-2]$) satisfying $\xleft(r'_i) = \xleft(B_\bm{t})$, and $\gbot(\bm{t})$ is the $r_i$ ($i \in [k-2]$) satisfying $\ybot(r'_i) = \ybot(B_\bm{t})$.
...
These sets have important properties, as proved in Appendix~\ref{app:hv}:

\begin{lemma} \label{lmm:hv:type1:properties}
    Let $r_i$ be the full rectangle of $r_i'$ for each $i \in [k-2]$. All the following statements are true:
    \myenums{
        \item $\cross(\bm{t}, H) \ne \emptyset$ and $\cross(\bm{t}, V) \ne \emptyset$.
        \item For any $h \in \cross(\bm{t}, H)$ and any $v \in \cross(\bm{t}, V)$, the $k$-tuple $(r_1, r_2, ..., r_{k-2}, h, v)$ belongs to $\J(R_1, ..., R_{k-2}, H, V)$.
        \item If a $k$-tuple $(r_1, r_2, ..., r_{k-2}, h, v)$ belongs to $\J_1$, then $h \in \cross(\bm{t}, H)$ and $v \in \cross(\bm{t}, V)$.
    }
\end{lemma}
...
Equipped Lemma~\ref{lmm:hv:type1:properties}, we can generate the target $S$ as follows:

\mytab{
    \> {\bf Algorithm Generate-$\J^*$} \\
    \> 1.\> $\J^* = \emptyset$ \\
    \> 2.\> {\bf for} each $(k-2)$-tuple $\bm{t} \in \J(R_1,$ $...,$ $R_{k-2}))$ {\bf do} \\
    \> 3.\>\> $r_i \leftarrow$ the full rectangle of $\bm{t}[i]$, for each $i \in [k-2]$ \\
    \>\>\> /* recall that $\bm{t}[i]$ is the $i$-th rectangle in $\bm{t}$ */ \\
