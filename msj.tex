...
We defer the proof of Lemma~\ref{lmm:hv:type1:recur-output} to the paragraph following the proof of Lemma~\ref{lmm:hv:type1:properties}, as the latter proof requires results in the former.

% \noindent {\bf Proof of Lemma~\ref{lmm:hv:type1:recur-output}.} We will prove the lemma by providing a way to map each $(k-2)$-tuple $\J(R_1',...,R_{k-2}')$ to a unique $k$-tuple in the H-V $k$-SJ result $\J(R_1,...,R_{k-2},H,V)$.
...
\extraspacing \noindent {\bf Proof of Lemma~\ref{lmm:hv:type1:recur-output}.} Our proof resorts to the algorithm generate-$\J^*$ (see Section~\ref{sec:hv:type1}). As we proved in Statement (3) of Lemma~\ref{lmm:hv:type1:properties}, both $\dcross_H(\bm{t})$ and $\dcross_V(\bm{t})$ are non-empty. Therefore, for each $\bm{t} \in \J(R_1',...,R_{k-2}')$, the algorithm generate-$\J^*$ will add to $\J^*$ at least one distinct tuple. This implies that $|\J(R_1',...,R_{k-2}')| \le |\J^*|\le \out$, where the second inequality follows from Statement (1) of Lemma~\ref{lmm:hv:type1:properties}.
...
We first prove Lemma~\ref{lmm:hv:type2:properties} and then proceed to prove Lemma~\ref{lmm:hv:type2:recur-output}, because the latter proof requires results in the former.

% \noindent {\bf Proof of Lemma~\ref{lmm:hv:type2:recur-output}.} We will prove the lemma by mapping each tuple of $\J(R_1,...,R_{k-3},H',V)$ to a unique tuple in $\J_2$.
...
\extraspacing \noindent {\bf Proof of Lemma~\ref{lmm:hv:type2:recur-output}.}
First, we prove that $\cross_{R_{k-2}}(\bm{t})$ is non-empty for each tuple $\bm{t} \in \J(R_1,...,R_{k-3},H',V)$. Let $\bm{t} = (r_1,...,r_{k-3},h',v)$. By the definition of a trimmed segment, there exists a rectangle $r_{k-2} \in R_{k-2}$ that covers both the left endpoint of $h$ and the right endpoint of $h'$. Thus, $r_{k-2}$ contains $h'$, because $h'$ shares the same left endpoint as $h$. Since the effective horizontal segment of $\bm{t}$ is contained in $h'$, it is also covered by $r_{k-2}$. This implies that $\cross_{R_{k-2}}(\bm{t})$ includes $r_{k-2}$, and is therefore non-empty. 

\vgap 

Then, we resorts to the algorithm generate-$\J_2$ (see Section~\ref{sec:hv:type2}). For each tuple $\bm{t} \in \J(R_1,...,R_{k-3},H',V)$, this algorithm adds to $\J_2$ exactly $|\cross_{R_{k-2}}(\bm{t})|\geq 1$ tuples. Therefore, $|\J(R_1, ..., R_{k-3}, H', V)| \le |\J_2| \le \out$, where the last inequality follows from the definition of $\J_2$ (see \eqref{eqn:hv:type2:J2}).
