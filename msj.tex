...
As mentioned in Section~\ref{sec:intro:prev}, 2-SJ can be solved in $F_2(n, \out) = O(n \log n + \out)$ time using a comparison-based algorithm. By Theorem~\ref{thm:main-recur}, in general, a comparison-based $(k-1)$-SJ algorithm with runtime $F_{k-1}(n, \out)$ spawns a comparison-based $k$-SJ algorithm whose running time $F_k(n, \out)$ obeys \eqref{eqn:main:reccurrence}. Specifically, for $k \ge 3$, there is a constant $c > 1$ such that
\myeqn{
    && F_{k}(n, \out) \nn \\
    ...
    && (c+c^2+c^3)\cdot k^3(k-1)^3(k-2)^3(n\log n + k \cdot\out) \nn\\
    &\leq& ... \nn \\
