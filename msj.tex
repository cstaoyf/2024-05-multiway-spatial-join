...
\extraspacing {\bf Algorithm for Problem $\bm{\mathscr{C}}$.} Let $S$ be a set of $N$ intervals in $\real$, each of which is associated with a real-valued {\em weight}. Given a search value $q$, a {\em stabbing max} query returns the maximum weight of all the intervals in $S$ covering $q$ (if no such intervals exist, the query returns $-\infty$). We can store $S$ in a structure of \cite{aak+12} using $O(N)$ space that can answer every query in $O(\log N)$ time. The structure also supports an insertion or deletion on $S$ in $O(\log N)$ amortized time.
