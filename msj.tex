...
Fix any rectangle $r \in R_1 \cup ... \cup R_{k-2}$ and any vertical segment $v$ from $V$ that crosses $r$. An argument symmetric to the above shows that $v$ must also cross $r'$.
...
\noindent \underline{\em Proof of Statement (3).} Our discussion will focus on explaining why $\sum_{\bm{t}\in \J(R_1',...,R_{k-2}')} |\dcross_H(\bm{t})|$ $\le \out$ because a similar argument can then used to prove $\sum_{\bm{t} \in \J(R_1',...,R_{k-2}')}$ $|\dcross_V(\bm{t})| \le \out$. 
 
\vgap 

To argue for $\sum_{\bm{t}\in \J(R_1',...,R_{k-2}')} |\dcross_H(\bm{t})|$ $\le \out$, we will provide a way to map each pair in $\J(R_1',...,R_{k-2}')\times \dcross_H(\bm{t})$ to a unique tuple in the H-V $k$-SJ result $\J(R_1,...,R_{k-2},H,V)$.

\vgap 

To explain the mapping, consider an arbitrary pair $(\bm{t},h) \in \J(R_1',...,R_{k-2}')\times \dcross_H(\bm{t})$. Recall that $B_\bm{t}$ is the intersection of the $k-2$ rectangles $\bm{t}[1], ..., \bm{t}[k-2]$. As $h \in \dcross_H(\bm{t})$, we know that $h$ crosses $B_\bm{t}$ and hence must intersect with the left edge of $B_\bm{t}$. Let $p$ denote that intersection point.

 Define $i$ to be the value in $[k-2]$ such that $\bm{t}[i] = \gleft(\bm{t})$. 
