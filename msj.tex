...
Define $r_1'$ the rectangle defined by the above four segments in the way illustrated in Figure~\ref{fig:four-segs}, where $r_1'$ is the shaded area. Note that $r_1'$ exists if and only if $r_1$ is crossed by at least one horizontal segment in $H$ and by at least one vertical segment in $V$. It is possible for $h_1$ and $h_2$ to be the same segment (same for $v_1$ and $v_2$).
...
a rectangle $r_1'$ to an initially empty set $R_1'$ if $r_1$ is crossed by at least one horizontal segment in $H$ and by at least one vertical segment in $V$. Specifically,
