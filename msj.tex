...
Fix an integer $k \ge 3$, as well as $k$ integers $n_1, n_2, ..., n_k$ at least 1. Define $n = \sum_{i=1}^k n_i$. An {\em $(n_1, n_2, ..., n_k)$-input} to the $k$-SJ problem comprises $k$ sets of rectangles $R_1, R_2, ..., R_k$ in $\real^2$.

\vgap

A {\em comparison-based $k$-SJ} algorithm for solving $(n_1, n_2, ..., n_k)$-inputs is a decision tree $\T$ defined as follows. Each internal node $u$ of $\T$ has three child nodes, referred as the {\em left}, {\em middle}, and {\em right} child, respectively. Each node is associated with an {\em instruction} of the form $(i_1, \dim_1, \lambda_1; i_2, \dim_2, \lambda_2)$.
...
\extraspacing {\bf Algorithm for Problem $\bm{\mathscr{E}}$.} Sort all the rectangles $r \in R$ in ascending order of $\xright(r)$ (namely, the x-coordinate of the right edge of $r$). Assign the ID $i \in [R]$ to $r$ if $r$ is at the $i$-th position of the sorted list.
