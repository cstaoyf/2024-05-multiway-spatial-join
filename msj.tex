...
In terms of techniques, our primary contribution is a result on the mathematical structure of the $k$-SJ problem. Specifically, for any $n \ge 1$ and $k \ge 2$, define function $F(n, k)$ to be the lowest value such that any instance of the $k$-SJ problem with input size $n$ and output size $\out$ can be solved in $F(n, k) + O(\out)$ time. Specificaly, define $F(n, 1) = n$ for any $n \ge 1$. Our main technical contribution can now be stated as: 
