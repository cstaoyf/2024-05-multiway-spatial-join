...
This paper revisits the {\em multiway spatial join} (MSJ) problem defined as follows. A rectangle $r$ in 2D space $\real^2$ is said to be {\em axis-parallel} if it has the form $r = [x_1, x_2] \times [y_1, y_2]$. Let $k$ be a constant integer at least 2. The input of MSJ comprises $k$ sets --- denoted as $R_1, R_2, ..., R_k$ --- of axis-parallel rectangles in $\real^2$. The objective is to report all the $k$-tuples $(r_1, r_2, ..., r_k)$ such that
\myitems{
...
We represent the set of such $k$-tuples as $\J_k(R_1, R_2, ..., R_k)$ and refer to it as the {\em join result}. Let $n = \sum_{i=1}^k |R_i|$, i.e., the input size, and $\out = |\J_k(R_1, R_2, ..., R_k)|$, i.e., the output size.
