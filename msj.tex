...
Suppose that $r' = [x_1,x_2] \times [y_1, y_2]$. Accordingly, $r^*$ can be written as $[x_1, x_2] \times [y_1, \maxtop(r')]$. We will first prove $\dcross_H(\bm{t})\subseteq \cross_H(r^*)$. For this purpose, given an arbitrary segment $h \in \dcross_H(\bm{t})$, we will show that $h$ crosses $r^*$. For this purpose, we need to explain why $\xleft(h) < x_1 \le x_2 < \xright(h)$ and $y_h \in [y_1, \maxtop(r')]$.
\begin{itemize}
    ...
    \item The fact $h \in \dcross_H(\bm{t})$ also tells us that $h$ crosses $B_{\bm{t}}$. Hence, $y_h \in [\ybot(B_\bm{t}), \ytop(B_{\bm{t}})]$. As $r' = \gbot(\bm{t})$, we know $\ybot(B_\bm{t}) = \ybot(r') = y_1$. By the definition of $\maxtop(r')$ in \eqref{eqn:hv:type1:maxtop}, we know $\ytop(B_{\bm{t}}) \leq \maxtop(r')$. It thus follows that $y_h \in [y_1, \maxtop(r')]$.
\end{itemize}
...
First, we prove that $h$ crosses $r'$. It suffices to show that $\xleft(h)  < x_1 \leq x_2 < \xright(h)$ and $y_h \in [y_1, y_2]$. Both conditions are satisfied since $h$ crosses $r^* = [x_1,x_2]\times [y_1, \maxtop(r')]$, and $\maxtop(r')\leq y_2$.

\vgap

Then, we prove that $h$ crosses $B_{\bm{t}}$. It suffices to show that $\xleft(h) < \xleft(B_{\bm{t}}) \le \xright(B_{\bm{t}}) < \xright(h)$ and $y_h \in [y_1, \ytop(B_{\bm{t}})]$.
\begin{itemize}
