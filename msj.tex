...
where $\J(R_1, ..., R_{k-2}, H, V)$, let us recall, is the join result of the (whole) H-V $k$-SJ. From $\J^*$, we will report only those $k$-tuples belonging to $\J_1$ and ignore the rest.
...
Define $r_1'$ as the rectangle bounded by the above four segments as shown in Figure~\ref{fig:hv:type1}(a) ($r_1'$ is the shaded area). Note that $r_1'$ exists if and only if $r_1$ is crossed by at least one horizontal segment in $H$ and by at least one vertical segment in $V$. Moreover, it is possible for $h_1$ and $h_2$ to be the same segment; same for $v_1$ and $v_2$. Define
\myeqn{
...
In the same fashion, we obtain the trimmed version $R_i'$ of $R_i$ for all $i \in [2, n]$, spending $O(k n \log n)$ time in total.
