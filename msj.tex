...
\extraspacing \underline{\em Proof of Statement (1).} We will map each segment $h^* \in H^*$ to a unique tuple $\bm{t}\in \J(R_1,...,R_{k-3},H',V)$ satisfying $\contained_{R_{k-2}}(\bm{t}) = \contained_{R_{k-2}}(h^*)$. We can then derive that
\myeqn{
