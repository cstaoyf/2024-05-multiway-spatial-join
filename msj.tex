...
\begin{lemma} \label{lmm:hv:type1:properties}
    ...
    \myenums{
        ...
        \item Consider any a $k$-tuple $(r_1, r_2, ..., r_{k-2}, h, v) \in \J_1$. Let $r'_i$ ($i \in [k-2])$ be the trimmed rectangle of $r_i$, and set $\bm{t} = (r'_1, r'_2, ...,$ $r'_{k-2})$. Then, we must have $\bm{t} \in \J(R_1', R'_2, ..., R'_{k-2})$, $h \in \dcross_H(\bm{t})$, and $v \in \dcross_V(\bm{t})$. 
...
\begin{lemma} \label{lmm:hv:type2:properties}
    ...
    \myenums{
        ...
        \myenums{
            ...
            \item Denote by $h$ the full segment of $\bm{t}[k-2]$. 
            For any $r \in \contained_{R_{k-2}}(\bm{t})$, the $k$-tuple $(\bm{t}[1], ..., \bm{t}[k-3], r, h, \bm{t}[k-1])$ belongs to $\J_2$.
        }
...
The correctness of the algorithm follows from statements (b) and (2) of Lemma~\ref{lmm:hv:type1:properties}. Furthermore, if we are given $\contained_{R_{k-2}}(\bm{t})$ for each $\bm{t}$, statement (3) of assures us that the algorithm runs in $O(1 + |\J(R_1, ..., R_{k-3}, H', V)| + k \sum_\bm{t} |\contained_{R_{k-2}}(\bm{t})|) = O(1 + k \cdot \out)$ time.
