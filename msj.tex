...
\underline{\em Proof of Statement (1).} By the definitions of $\dcross_H(\bm{t})$ and $\dcross_V(\bm{t})$, both segments $h$ and $v$ cross $B_\bm{t}$. Thus, the intersection of $h$ and $v$ is a point $p$ in $B_\bm{t}$. As $r_i$ (for each $i \in [k-2]$) is the full rectangle of $\bm{t}[i]$, we know that $\bigcap_{i=1}^{k-2} r_i$ covers $\bigcap_{i=1}^{k-2} \bm{t}[i]$, which is $B_\bm{t}$. Hence, point $p$ falls in $\bigcap_{i=1}^{k-2} r_i$, indicating that $h\cap v \cap \bigcap_{i = 1}^{k-2}r_i \neq \emptyset$. It follows that $(r_1,...,r_{k-2},h,v)$ is a result tuple in $\J(R_1,...,R_{k-2},H,V)$.

\vgap


\extraspacing {\bf Computing $\bm{R^*_1, R^*_2, ..., R^*_{k-2}}$.} We consider, w.l.o.g., that each rectangle $r$ in the input $R_1 \cup ... \cup R_{k-2}$ is given a distinct integer ID in $[n]$. This allows us to create an array $A$ of size $n$ and store in $A[i]$ ($i \in [n]$) a constant amount of information for each $r$.

\vgap

To compute $R^*_1, R^*_2, ..., R^*_{k-2}$, we start by deriving $\maxtop(r')$ for each rectangle $r'$ in $R'_1 \cup  ... \cup R'_{k-2}$. For this purpose, first initialize $\maxtop(r') = -\infty$ for each such $r'$. Recall that $r'$ is the trimmed rectangle of some rectangle $r$ in $R_1 \cup ... \cup R_{k-2}$. We store $\maxtop(r')$ in the cell of array $A$ corresponding to $r$; the cell can be accessed in $O(1)$ time using the ID of $r$.

\vgap

Then, we scan $\J(R_1',...,R_{k-2}')$. For each tuple $\bm{t}$ therein, use $O(k)$ time to identify the rectangle $r' = \gbot(\bm{t})$, and then update $\maxtop(r')$ to the maximum between its current value and $\ytop(r')$. The scan requires $O(n + k \cdot \out)$ time.

\vgap

Finally, for each $i \in [k-2]$, we construct $R_i^*$ by collecting the top-sliced rectangle of every rectangle $r' \in R_i'$ (see definition in \eqref{eqn:hv:type1:topsliced}) with $\maxtop(r') \neq -\infty$. This step takes $O(|R_i'|)$ time for each $i \in [k-2]$, or $O(n)$ total time for all $i$.
