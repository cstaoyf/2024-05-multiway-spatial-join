...
%         \includegraphics[height=30mm]{./artwork/prob-d} \\
        (a) Problem $\EuScript{A}$ and $\EuScript{D}$ &
        \hspace{3mm}
...
This subsection will explain how to find the $k$-tuples in $\J(R_1, ..., R_{k-2},$ $H, V)$ that belong to Type 1. We will represent the set of such tuples as $\J_1(R_1, ..., R_{k-2}, H, V)$ and define its ``projection'' onto $(R_1, ..., R_{k-2})$ as
\myeqn{
...
\begin{figure}
    ...
    \figcapup
    \caption{Constructing $r_1'$ from $r_1$}
    \label{fig:four-segs}
    \figcapdown
\end{figure}


\noindent {\bf Step 1.} We will accomplish this step by solving an instance of $(k-2)$-SJ constructed as follows. For each rectangle $r_1 \in R_1$, we compute four segments:
\myitems{
...
Define $r_1'$ as the rectangle defined by the above four segments in the way illustrated in Figure~\ref{fig:four-segs}, where $r_1'$ is the shaded area. Note that $r_1'$ exists if and only if $r_1$ is crossed by at least one horizontal segment in $H$ and by at least one vertical segment in $V$. Moreover, it is possible for $h_1$ and $h_2$ to be the same segment; same for $v_1$ and $v_2$. Define
\myeqn{
...
We now solve the $(k-2)$-SJ problem on $R_1', R_2, ..., R_{k-2}$ using the algorithm $\A$ supplied. Let us represent the result of this $(k-2)$-SJ as $\J(R_1', R_2, ..., R_{k-2})$. The following observation is crucial.

\begin{lemma} \label{sec:hv:type1:proj}
    A $(k-2)$-tuple $(r_1', r_2, ..., r_k)$ exists in $\J(R_1', R_2, ..., R_{k-2})$ if and only if the $(k-2)$-tuple $(r_1, r_2, ..., r_k)$ exists in $\Pi_{1:k-2}(\J_1(R_1, ...,$ $R_{k-2}, H, V))$, where $r_1$ is the rectangle in $R_1$ defining $r_1'$.
\end{lemma}


\subsection{Finding Result Tuples of Type 2} \label{sec:hv:type2}

\subsection{Remarks} \label{sec:hv:remarks}
