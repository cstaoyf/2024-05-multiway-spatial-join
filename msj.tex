...
\myitems {
    \item First, we explain why $h$ crosses $r'$. Because $r'$ covers $r^*$, by the fact of $h$ crossing $r^*$, we know that $h \cap r' \ne \emptyset$. On the other hand, as $h$ crosses $r^*$, it must hold that $\xleft(h) < x_1 \le x_2 < \xright(h)$ (recall that $[x_1, x_2]$ is the x-range of $r^*$). As $[x_1, x_2]$ is also the x-range of $r'$, the fact of $h \cap r' \ne \emptyset$ indicates that $h$ must cross $r'$.
