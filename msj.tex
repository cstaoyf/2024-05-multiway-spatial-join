...
% }
\myeqn{
    \dcross(\bm{t}, H) = \set{h \in H \mid \text{$h$ crosses both $B_\bm{t}$ and $\gleft(\bm{t})$}} \nn \\
    \dcross(\bm{t}, V) = \set{h \in H \mid \text{$v$ crosses both $B_\bm{t}$ and $\gbot(\bm{t})$}}. \nn
}
...
\begin{lemma} \label{lmm:hv:type1:properties}
    ...
    \myenums{
        ...
        \myenums{
            \item $\dcross(\bm{t}, H) \ne \emptyset$ and $\dcross(\bm{t}, V) \ne \emptyset$.

            \item Denote by $r_i$ ($i \in [k-2]$) the full rectangle of $\bm{t}[i]$. 
            For any $h \in \dcross(\bm{t}, H)$ and any $v \in \dcross(\bm{t}, V)$, the $k$-tuple $(r_1, r_2, ..., r_{k-2}, h, v)$ belongs to $\J(R_1, ..., R_{k-2}, H, V)$.

            \item If the $k$-tuple $(r_1, r_2, ..., r_{k-2}, h, v)$ belongs to $\J_1$, then $h \in \dcross(\bm{t}, H)$ and $v \in \dcross(\bm{t}, V)$.
        }

        \vspace{2mm}

        \item $\sum_{\bm{t}} |\dcross(\bm{t}, H)| \le \out$ and $\sum_{\bm{t}} |\dcross(\bm{t}, V)| \le \out$.
    }
\end{lemma}
...
\mytab{
    ...
    \> 4.\>\> {\bf for} each $(h, v) \in \dcross(\bm{t}, H) \times \dcross(\bm{t}, V)$ {\bf do} \\
    \> 5.\>\>\> add $(r_1, ..., r_{k-2}, h, v)$ to $\J^*$
}


Lemma~\ref{lmm:hv:type1:properties} assures us that the set $\J^*$ thus computed indeed satisfies \eqref{eqn:hv:type1:J*}. Furthermore, if we are given $\dcross(\bm{t}, H)$ and $\dcross(\bm{t}, V)$ for each $\bm{t}$, Lemma~\ref{lmm:hv:type1:properties} further asserts that the above algorithm runs in $O(1 + k \cdot |\J^*|) = O(1 + k \cdot \out)$ time.

\vgap 

The rest of the section will focus on explaining how to prepare the sets $\dcross(\bm{t}, H)$ of all $\bm{t} \in \J(R_1', ..., R_{k-2}')$ in $O(n \log n + k \cdot \out)$ time. An analogous algorithm can be used to compute the sets $\dcross(\bm{t}, V)$ of all $\bm{t}$ in the same time complexity.

\extraspacing {\bf Sets $\bm{R^*_1, R^*_2, ..., R^*_{k-2}}$.} Fix any $i \in [k-2]$. For each rectangle $r' \in R_i'$, we define
\myeqn{
    \maxtop(r') &=& \max_{\bm{t} \in \J(R'_1, ..., R'_{k-2}): r' = \gbot(\bm{t})} \xright(B_\bm{t}). 
}
...
We refer to $r^*$ as the {\em top-sliced rectangle} of $r'$.

\yf{example}

\vgap 

Now construct from $R_i'$ a new set of rectangles: 
\myeqn{
