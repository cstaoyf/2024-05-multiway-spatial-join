...
\myeqn{
    ...
    \dcross_V(\bm{t}) = \set{v \in V \mid \text{$v$ crosses both $B_\bm{t}$ and $\gbot(\bm{t})$}}. \nn
    %\label{eqn:dcross-H} 
}
...
We 
conclude that $\J_1$ (the set of type-1 result tuples) can be computed in $F_{k-1}(n, \out) + O(n \log n +  k \cdot \out)$ time.
