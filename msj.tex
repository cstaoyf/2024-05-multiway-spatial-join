...
\myitems{
    \item {{\bf Cat.\ 1:}} $h$ crosses all of $r_1, ..., r_{k-2}$, and $v$  crosses all of $r_1, ..., r_{k-2}$; 
    
    \item {{\bf Cat.\ 2:}} $h$ or $v$ fails to cross some rectangle in $\set{r_1, r_2, ..., r_{k-2}}$. This means the existence of at least one rectangle $r_i$, for some $i \in [k-2]$, covers either an endpoint of $h$, or an endpoint of $v$, or both.
}
