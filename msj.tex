...
We now prove the first claim of statement (2), i.e., $\bm{t}\in \J(R_1',...,R_{k-2}')$. Consider the $k$-tuple $(r_1,...,r_{k-2},h,v)\in \J_1$ given in statement (2). By definition of $\J_1$, segments $h$ and $v$ both cross each of the rectangles $r_1,..., r_{k-2}$. As explained above, $h$ and $v$ must also cross each of the trimmed rectangles $r_1',..., r_{k-2}'$. Hence, $h \cap v$ is a point covered by each of $r_1',..., r_{k-2}'$. Therefore, $B_\bm{t} = \bigcap_{i = 1}^{k-2}r_i'$ cannot be empty, meaning that $\bm{t} = (r_1', ..., r_{k-2}') \in \J(R_1',...,R_{k-2}')$.

\vgap 

Next, we prove the second claim, i.e., $h \in \dcross_H(\bm{t})$ and $v \in \dcross_V(\bm{t})$. It suffices to show only the former due to symmetry. To prove $h \in \dcross_H(\bm{t})$, we need to argue that $h$ crosses $\gbot(\bm{t})$ and $B_\bm{t}$. The first part, $h$ crossing $\gbot(\bm{t})$, is done because as mentioned $h$ crosses each of $r_1', ..., r_{k-2}'$, and $\gbot(\bm{t})$ is one of those rectangles. To prove that $h$ crosses $B_\bm{t}$, first note that $h \cap B_\bm{t} \ne \emptyset$ because as explained before $B_\bm{t}$ contains the point $h \cap v$. On the other hand, as $\gbot(\bm{t})$ covers $B_\bm{t}$, the fact of $h$ crossing $\gbot(\bm{t})$ indicates that the x-range of $h$ contains that of $B_\bm{t}$. Combining this with $h \cap B_\bm{t} \ne \emptyset$ shows that $h$ crosses $B_\bm{t}$.
