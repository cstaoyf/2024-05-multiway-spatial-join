...
\myeqn{
    ...
    \dcross_V(\bm{t}) = \set{h \in H \mid \text{$v$ crosses both $B_\bm{t}$ and $\gbot(\bm{t})$}}. \label{eqn:dcross-H} 
}
...
The rest of the section will focus on explaining how to prepare the sets $\dcross_H(\bm{t})$ of all $\bm{t} \in \J(R_1', ..., R_{k-2}')$ in $O(n \log n + k \cdot \out)$ time. An analogous algorithm can be used to compute the sets $\dcross_V(\bm{t})$ of all $\bm{t}$ in the same time complexity.
