...
Recall that $\J(R_1, ..., R_{k-2}, H, V)$ is the result of the original H-V $k$-SJ problem, which has a size of $\out$. Only the $k$-tuples in $S$ that are of Type 1 will be reported. We will achieve the purpose in $O(n \log n + |S|)$ time.
...
which is $O(1 + \out)$ because of \eqref{eqn:hv:type1:S}. In what follows, we will explain how to compute all the $\cross(r_a, B; H)$ in $O(n \log n + |S|)$ time. A symmetric algorithm can be used to compute 
