...
The value of $\maxright(r')$ does not exist if no $\bm{t} \in \J(R'_1, ..., R'_{k-2})$ has $r'$ as the $\gbot(\bm{t})$. When $\maxright(r')$ exists, assuming $r' = [x_1, x_2] \times [y_1, y_2]$, we introduce a rectangle
\myeqn{
    r^* &=& [x_1, \maxright(r')] \times [y_1, y_2]. \nn
}
We refer to $r^*$ as the {\em star rectangle} of $r'$.

\yf{example}

\vgap 

Now construct from $R_i'$ a new set: 
\myeqn{
    R^*_i = \set{r* \mid r' \in R_i' \text{ and $r^*$ defined as above exists}}. \nn
}
