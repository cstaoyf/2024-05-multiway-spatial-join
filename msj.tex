...
We now solve a $(k-1)$-SJ problem with $R_1, ..., R_{k-1}, H', V$ as the input using the algorithm $\A$ supplied (see Lemma~\ref{lmm:hv}). This $(k-1)$-SJ has an input size at most $n$, and let us represent its result as $\J(R_1, ..., R_{k-1}, H', V)$. We prove in Appendix~\ref{app:hv}:
...
\extraspacing {\bf Generating $\bm{\J^*}$.}
