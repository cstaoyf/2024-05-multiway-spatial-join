...
At a leaf, we look at its associated result and, for each $(\lambda_1, \lambda_2, ..., \lambda_k)$ in that result, output $(r_1, r_2, ..., r_k)$ where $r_i$ ($i \in [k]$) is the rectangle with ID $\lambda_i$ in $R_i$. The algorithm is {\em correct} if it produces the correct output on all $(n_1, n_2, ..., n_k)$-inputs. Its {\em worst-case cost} is the height of $\T$.


\vgap

Through a reduction from the element distinctness problem \cite{dl79}, one can show that, for $k = 2$, any comparison-based algorithm capable of solving $(n/2, n/2)$-inputs must have a worst-case cost of $\Omega(n \log n)$.
