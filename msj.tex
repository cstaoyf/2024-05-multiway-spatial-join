...
\myitems{
    ...
    \item {{\bf Type 2:}} $h$ or $v$ fails to cross some rectangle in $\set{r_1, r_2, ..., r_{k-2}}$. This means the existence of at least one rectangle $r_i$, for some $i \in [k-2]$, covers either an endpoint of $h$, or an endpoint of $v$, or both.
}
Figure~\ref{fig:hv:cats} illustrates a result tuple of each type, assuming $k = 4$.
