...
From the perspective of theory, SJs are best understood when $k = 2$, i.e., the {\em pairwise} scenario, where the problem can be solved in $O(n \log n + \out)$ time with planesweep \cite{bcko08}. However, the problem becomes significantly more challenging for $k \ge 3$, i.e., the {\em multiway} scenario. All the solutions developed (see \cite{mp98,mp01,pmt99,???} and the references therein) before 2022 suffer from a worst-case time complexity of $O(n^k)$, offering essentially no improvement over the naive method that simply enumerates the entire cartesian product $R_1 \times R_2 \times ... \times R_k$.
