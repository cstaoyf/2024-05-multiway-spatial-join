...
Next, we prove the first bullet of statement (2), namely, $\bm{t}\in \J(R_1',...,R_{k-2}')$. Consider the $k$-tuple $(r_1,...,r_{k-2},h,v)\in \J_1$ given in statement (2). By definition of $\J_1$, segments $h$ and $v$ both cross each of the rectangles $r_1,..., r_{k-2}$. By the earlier discussion, $h$ and $v$ also both cross each of the trimmed rectangles $r_1',..., r_{k-2}'$. This means that the intersection of $h$ and $v$ must be a point in each of $r_1',..., r_{k-2}'$, suggesting $\bigcap_{i = 1}^{k-2}r_i' \neq \emptyset$. It thus holds that $\bm{t} = (r_1', ..., r_{k-2}')$ is a result tuple in $\J(R_1',...,R_{k-2}')$. 
