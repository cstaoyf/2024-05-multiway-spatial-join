...
\begin{table}
    \begin{tabular}{c|c|c|c}
        ...
        $\ge 3$ & \cite{kcko22} & $O(n \log^{\red{2k-1}} n + \out)$ & \\
        \hline
...
\myeqn{
    B_\bm{t} &=& \bigcap_{i=1}^{\red{k}} r_i; \label{eqn:B_t}
}
...
In the {\em reporting version} of Problem $\mathscr{A}$, the goal is to output, for each point $p \in P$, all the rectangles $r \in R$ containing $p$; if no such $r$ exists, report nothing for $p$. In Figure~\ref{fig:guard}a, for instance, the output is $\set{(p_2: r_1, r_2), (p_3: r_2)}$. As shown in Appendix~\ref{app:bricks}, \red{the} problem can be solved in $O(n \log n + \out)$ time, where $\out$ is the number of pairs $(p, r) \in P \times R$ such that $p \in r$.
...
\myeqn{
    \J_2  &=& \{(r_1, ..., r_{k-2}, h, v) \in \J(R1, ..., R_{k-2}, H, V) \mid \text{$r_{k-2}$ covers the left endpoint of $h$}\}. \label{eqn:hv:type2:J2}
}
