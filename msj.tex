...
\myitems{
    ...
    \item $\gleft(\bm{t})$ is the $r_i$ ($1 \le i \le k-2$) with $\xleft(r'_i) = \xleft(B_\bm{t})$;
    \item $\gbot(\bm{t})$ is the $r_i$ ($1 \le i \le k-2$) with $\ybot(r'_i) = \ybot(B_\bm{t})$.
}
We now introduce:
% \myitems{
...
Lemma~\ref{lmm:hv:type1:properties} assures us that the set $\J^*$ thus computed indeed satisfies \eqref{eqn:hv:type1:J*}. Furthermore, if we are given $\cross(\bm{t}, H)$ and $\cross(\bm{t}, V)$ for each $\bm{t}$, Lemma~\ref{lmm:hv:type1:properties} further asserts that the above algorithm runs in $O(1 + |\J^*|) = O(1 + \out)$ time.
