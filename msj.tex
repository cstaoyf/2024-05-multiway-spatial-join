...
Equipped Lemma~\ref{lmm:hv:type1:properties}, we can generate our target $\J^*$ as follows:
...
\noindent Lemma~\ref{lmm:hv:type1:properties} assures us that the set $\J^*$ thus computed indeed satisfies \eqref{eqn:hv:type1:J*}. Furthermore, if we are given $\cross(\bm{t}, H)$ and $\cross(\bm{t}, V)$ for each $\bm{t}$, Lemma~\ref{lmm:hv:type1:properties} further asserts that the above algorithm runs in $O(1 + |\J^*|) = O(1 + \out)$ time.
