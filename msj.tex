...
In \cite{kcko22}, Khamis et al.\ investigated a type of joins that extend equi-joins in two ways. First, each attribute value in a relation is an interval (rather than a real value); second, each equality predicate in equi-joins is replaced with an ``non-empty intersection'' predicate on the attributes involved. The $k$-SJ problem can be regarded as a join defined next under the framework of \cite{kcko22}. For each $i \in [k]$, define $R_i$ as a relation over two attributes $X$ and $Y$. For each tuple $\bm{u} \in R_i$, its values $\bm{u}(X)$ and $\bm{u}(Y)$ on the two attributes are both intervals (effectively defining a rectangle). The objective is to output all $(\bm{u}_1, \bm{u}_2, ..., \bm{u}_k) \in R_1 \times R_2 \times ... \times R_k$ such that $\cap_{i=1}^k \bm{u}_i(X) \ne \emptyset$ and $\cap_{i=1}^k \bm{u}_i(Y) \ne \emptyset$.
