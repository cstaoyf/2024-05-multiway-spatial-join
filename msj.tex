...
Now, we compute $\cross_H(r^*)$ for each $r^* \in R^*_i$ --- where $\cross_H(r^*)$, defined in \eqref{eqn:cross}, is the set of segments in $H$ crossing $r^*$ --- which is an instance of the find-all-sorted version of Problem $\mathscr{D}$. The following lemma, proven in Appendix~\ref{app:hv}, presents some crucial properties for these ``cross'' sets. 
...
Statement (1) of the above lemma tells us that 
